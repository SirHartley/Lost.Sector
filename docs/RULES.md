...
| `nskr_kqIntelTitle`, `nskr_kqIntelBullets`, `nskr_kqIntelDesc` | Text of quest `kq`'s intel entries, read by `quest/QuestText` outside any dialog ([Intel](../jars/src/lostsector/quest/README.md#intel)). Job 1 (key `job1`) is the `# KESTEVEN QUESTLINE: JOB 1` block, job 3 (key `job3`) the `# KESTEVEN QUESTLINE: JOB 3` block; their checks and tokens are declared by `KestevenJob1Module` and `KestevenJob3Module`. [Job 1](quests/KESTEVEN_QUESTLINE.md#job-1-enemy-unknown-stages-0-to-6), [Job 3](quests/KESTEVEN_QUESTLINE.md#job-3-hostile-takeover-stages-6-to-11) |
