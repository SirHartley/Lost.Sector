...
Every other Leave is vanilla's `defaultLeave`, which also takes Escape. A screen's paragraphs are one Text cell separated by blank lines, with one `SetTextHighlights` for the cell; `AddText` remains for grey paragraphs (`AddText … gray`) and for text after an insert, a receipt or a quest verb. The rows set no font; vanilla's `AddTextSmall` prints its line in the small insignia font and switches to the insignia font after it (0.98a-RC8 `AddTextSmall.execute`).
...
| Eliza's intercept (stage 19) | `nskr_kq_elizaInterceptInit` and `…Greeting` on role flag `$nskr_kq_elizaIntercept`, `…Talk`, the `FireAll` menu `nskr_kqElizaInterceptOptions` (`…OptHandOver` and `…OptRefuse` unless `check elizaChipGone`, `…OptNoChip` with it), `…HandOver`, `…Refuse`, `…NoChip`, `…Exit`. No row tests the returning role `$nskr_kq_elizaReturning`, so the fleet flying home after the hand-over gets the vanilla fleet rows | `nskr_quest kq do elizaAggro`, `elizaTalked`, `elizaChipHandOver`, `elizaHostile`; token `elizaHomeMarket` |
