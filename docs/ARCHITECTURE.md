...
| [Quest framework](../jars/src/lostsector/quest/README.md) | How quest code is built: state, stages, modules, events, fleets, people, intel, rules contract, dev tools, rules check tool |
| [Quest implementation](quests/README.md) | Current behavior of each quest: Kesteven questline, contracts, bounties and intercepts, blacksites, Hellspawn judgement, exploration hints |

## Start here

| Change / symptom | Route |
|---|---|
| Kesteven questline stage or job | Quest `kq` (`kesteven/quest/KestevenQuest`, state `KestevenState`): rows in `rules.csv` `# KESTEVEN QUESTLINE` and its sub-blocks `-> nskr_quest kq ->` the module that declares the check, action or token ([modules](#kesteven-questline-contracts-and-hints)); stage changes `nskr_quest kq advance -> quest/QuestManager`. [Walkthrough](quests/KESTEVEN_QUESTLINE.md), [saved state](quests/KESTEVEN_STATE.md), [dialogue map](quests/KESTEVEN_DIALOGUE.md) |
| Quest entity or market opens the wrong dialog | `CorePlugin.pickInteractionDialogPlugin`: claimed entities (`quest/QuestDialogs.CLAIM_KEY`, such as the blacksites, the data-disk satellites and the Cache core) -> rules dialog on the claimed trigger; the Hellspawn judgement and Gate Conduit encounters. Markets whose whole dialog a quest scene replaces (Eliza's port, the endings) -> `OpenInteractionDialog` rows at `score:10000` gated by a `nskr_quest kq check` ([project routing](RULES.md#project-routing)) |
| Endings and the production chip | `rules.csv` `# KESTEVEN QUESTLINE: ENDINGS` rows `-> kesteven/quest/KestevenEndingsModule`, `# KESTEVEN QUESTLINE: ALTERNATIVE ENDINGS` rows `-> kesteven/quest/KestevenAltEndingsModule -> KestevenEndingsModule.unlockSettings()`; `kesteven/quest/UnlimitedProductionChipCondition -> kesteven/BlackOpsManager.getUPC()` |
...
| Event fleets | Quest `ic` in `events/intercepts` (ARO strike, "LZ" messenger, Auto-Hunter, Kesteven debt collector; modules `quest/modules/InterceptEncounter` and `PayOffEncounter`, rows `# INTERCEPTS`); the Tri-Tachyon collector of quest `kq` (`kesteven/quest/KestevenCollector`, the same two modules, rows `# KESTEVEN QUESTLINE: COLLECTOR`); the questline's fleet builders in `kesteven/quest/KestevenFleets`; Eliza's fleets in `kesteven/quest/KestevenElizaFleetsModule`; dialogue in `rules.csv` |
| Debt, ship swap, S-mod removal | Official menus in `rules.csv -> dialogue/rules/nskr_debt`, `nskr_shipSwap`, `nskr_modRemoval`; monthly interest `kesteven/loans/CrushingDebt`. The rows hold the entry options, the service menus and the handlers; the commands print the rest of each service: option lists, previews, picker results, confirmations and receipts ([project routing](RULES.md#project-routing)) |
...
|---|---|---|
| Transient `EFS_LIST` | Built anew by every `onGameLoad`, for new games and loaded saves: `HyperspaceEnigmaSpawner`, `enigma/HeartOccupation`, `StalkerSpawner`, `enigma/EnigmaRelations`, `KestevenScavenger`, `kesteven/KestevenExportManager`, `GuardSpawner`, `kesteven/ExileManager`, `BlackOpsManager`, `ContractManager`, `enigma/EnigmaHullmodListener`, `enigma/EnigmaAIConverter`, `GameModeManager`, `ThronesGiftManager`, `HellSpawnManager`, `quest/QuestManager`; `HellSpawnNexListener` only with Nexerelin | Each is a `BaseCampaignEventListener` and `EveryFrameScript`, advanced every campaign frame (`QuestManager` skips paused frames). The loop also adds each to the listener manager, so any listener interface the class implements is called, as for the listeners of `QuestManager` ([Events](../jars/src/lostsector/quest/README.md#events)). Removed before each save and re-added after it. Constructors may read the loaded sector (`CampaignTimer` reads its stored value only there); `persistence/Saved` fields are loaded after construction. Keep per-save state in instance fields, `Saved` or persistent data, not in static fields. |
...
`CorePlugin` is a transient `BaseCampaignPlugin`: vanilla drops transient plugins when saving, so `onGameLoad` registers it again. Its `pickInteractionDialogPlugin` routes entities claimed through `quest/QuestDialogs.CLAIM_KEY` to a rules dialog and the Hellspawn fleets to their fleet encounter plugins.

Combat listeners added to ships and entities last for one battle.
...
Mod assets live under `graphics/lostsector/` and `sounds/lostsector/`; paths outside those folders refer to vanilla assets. IntelliJ compiles `jars/src` to `jars/production` and builds the `jars/Lost.Sector.jar` artifact from that module output (`.idea/artifacts/Lost_Sector_jar.xml`). The repository has no `.idea/compiler.xml`, so IntelliJ uses its default resource patterns (`!?*.java;!?*.form;!?*.class;!?*.groovy;!?*.scala;!?*.flex;!?*.kt;!?*.clj;!?*.aj`, `DEFAULT_WILDCARD_PATTERNS` in JetBrains' `JpsJavaCompilerConfigurationSerializer`): every other file under `jars/src` is copied into the output and into the jar. `quest/README.md` therefore ships inside `Lost.Sector.jar`; data files that tools read by path, such as the rules check's `docs/rules-reference/vanilla-rules-index.txt`, stay outside `jars/src`. Build procedure: [CLAUDE.md](../CLAUDE.md#building).
...
|---|---|
| `IS_NEXERELIN` (`nexerelin`) | World generation mode, `HellSpawnNexListener`, diplomacy calls in `kesteven/quest/KestevenEndingsModule`, `HellSpawnManager`, `kesteven/ExileManager`, `SectorGen`. Also cleared in `onGameLoad` when `SectorManager.getManager()` is null. `HellSpawnBackground`/`ThronesGiftBackground` extend Nexerelin's `BaseCharacterBackground` and are reached only through Nexerelin's background CSV. |
...
| `IS_IRONSHELL` (`timid_xiv`) | `kesteven/quest/KestevenEndingsModule` |
...
| LunaLib `LunaSettings/lost.sector.json` | Per installation, outside saves, keyed by CSV `fieldID`; shared by all campaigns on that machine, including the unlocks from `KestevenEndingsModule.unlockSettings()` | A `Setting` constant's name is its `fieldID`; renaming either drops the stored value. |
...
| `world/systems/cache/Cache` | System `Unknown Site`: gate `nskr_cacheGate`, `nskr_cache_derelict1`-`4`, `nskr_cache_core`; the guardian's builder (`guardianFleet`, `finishGuardianFleet`) and encounter config, spawned by `KestevenCacheModule` |
| `world/systems/outpost/Outpost` | In a random system near the core: `nskr_outpost`, `nskr_outpost_gate`, `nskr_outpost_relay` |
| `world/SectorGen` | People at fixed markets through `genPeople()`; `genEliza()` (`nskr_anarchist`) is called from `KestevenElizaModule`: its action `elizaMeet` and its `onSkip` past `JOB5_DISKS`, which the story skip's jump runs |
...
|---|---|
| `CorePlugin` | Transient campaign plugin; `pickInteractionDialogPlugin` routes claimed entities to rules dialogs and custom-start fleets to their encounter plugins |
...
| File | Owner / connection |
|---|---|
| `kesteven/quest/KestevenQuest`, `KestevenStage`, `KestevenFlag`, `KestevenState` | Quest `kq` in `quest/QuestCatalog`. `KestevenState` holds every saved questline value; `QuestManager` is the only stage writer. Features outside the questline (`CorePlugin`, `ContractManager`, `ExileManager`, `StalkerSpawner`, `events/intercepts/InterceptsQuest`, `world/systems/cache/Cache`, `nskr_debt`, `nskr_shipSwap`, `nskr_modRemoval`) use only `KestevenQuest`'s public static [queries and actions](quests/KESTEVEN_STATE.md#queries-for-other-features). [State map](quests/KESTEVEN_STATE.md) |
| `kesteven/quest/*Module`, `KestevenCollector` | The modules of quest `kq`, in module order; see the table below |
...
| `kesteven/quest/KestevenFleets`; `helper/fleet/SimpleFleet`, `SimpleFleetMember`, `SimpleCaptain`, `SystemPicker`, `FleetInfo` | Questline fleet builders returning `SimpleFleet`; fleet and system builders shared by all spawners |
...
Modules of quest `kq`, in module order. Each declares the checks, actions and tokens of the `rules.csv` block named in its row.

| Module | Active in | Owns |
|---|---|---|
| `KestevenHubModule` | every stage | Every conversation with Jack, Alice and Nicholas (`# KESTEVEN QUESTLINE`): job gates, payouts, the story skip |
| `KestevenJob1Module` | `NOT_STARTED` to `JOB3_OFFERED` | The sensor task (`onEncounterLoot`), the tip system's dormant fleet (role `job1Dormant`), intel `job1` (`# KESTEVEN QUESTLINE: JOB 1`) |
| `KestevenJob3Module` | `JOB3_ACTIVE` to `JOB3_DONE` | The target's dormant fleet and satellite #3, the expedition (roles `job3Expedition`, `job3ExpeditionOver`, `FleetOrders.expedition`), success, timeout and stealth failure, intel `job3` (`# KESTEVEN QUESTLINE: JOB 3`) |
| `KestevenPartyModule` | `JOB3_ACTIVE`, `JOB3_TARGET_KNOWN` | The party bar event's guests, drinks and hangover (`# KESTEVEN QUESTLINE: JOB 3 PARTY`) |
| `KestevenJob4Module` | `JOB4_WAITING` to `JOB5_DISKS` | The wait (timer `job4Wait`), the strike group, Special Operations and splinter fleets (roles `job4StrikeGroup`, `job4SpecialOps`, `job4SpecialOpsLeaving`, `job4Splinter`) and their sightings (`onFleetDetected`), the wrecks, completion, the failure for attacking the Special Operations fleet, intel `job4` (`# KESTEVEN QUESTLINE: JOB 4`) |
| `KestevenJob5Module` | `JOB5_MEETING` to `COMPLETED` | The Delve meeting bar event and its guard (person `delveGuard`), intel `job5` (`# KESTEVEN QUESTLINE: JOB 5`) |
| `KestevenGlacierModule` | `JOB5_DISKS` to `COMPLETED`, `FAILED` | Glacier's mark and claim (trigger `nskr_kqGlacier`), the barrage damage, disk #5 (`# KESTEVEN QUESTLINE: GLACIER`) |
| `KestevenElizaSearchModule` | `JOB5_DISKS` | The search at pirate bars: three bar events and their quest people, the contact market and its move on decivilization (`# KESTEVEN QUESTLINE: ELIZA SEARCH`) |
| `KestevenSatelliteModule` | every stage | The claim on every data-disk satellite, salvage, the woken guard (role `satelliteGuard`, `FleetOrders.huntInSystem`), `ALL_DISKS_RECOVERED`, `FROST_FOUND` (`# KESTEVEN QUESTLINE: SATELLITES`) |
| `KestevenElizaModule` | every stage | Eliza's port, which replaces her market's dialog until the meeting has finished, the raid objective `elizaDisks` after a refusal, her move on decivilization (`# KESTEVEN QUESTLINE: ELIZA`) |
| `KestevenCollector` | every stage | The Tri-Tachyon collector: an `InterceptEncounter` (roles `ttCollector`, `ttCollectorLeaving`) and a `PayOffEncounter` (`nskr_electronics`, everything held) (`# KESTEVEN QUESTLINE: COLLECTOR`) |
| `KestevenElizaFleetsModule` | every stage | Eliza's raided, intercept and revenge fleets (roles `elizaRaided`, `elizaIntercept`, `elizaReturning`, `elizaRevenge`), `ELIZA_KILLED`, `ELIZA_BETRAYED`, the chip hand-over (`# KESTEVEN QUESTLINE: ELIZA FLEETS`) |
| `KestevenEndingsModule` | every stage | The Kesteven and Eliza endings, which replace the dialog of the home market and of Eliza's market while each is available; the commission restore after the Eliza ending (`wantsFrames`) and its relationship caps (`onReputationChange`); `unlockSettings()` (`# KESTEVEN QUESTLINE: ENDINGS`) |
| `KestevenAltEndingsModule` | `CHIP_RECOVERED`, `COMPLETED` | The Luddic and Tri-Tachyon endings: market checks, price, payment, fallout, the chip's market (`# KESTEVEN QUESTLINE: ALTERNATIVE ENDINGS`) |
| `KestevenAftermathModule` | every stage | Jack's revenge (role `jackRevenge`, `JACK_GONE`) and the failure once Kesteven holds neither the Outpost nor Asteria (`# KESTEVEN QUESTLINE: AFTERMATH`) |
| `KestevenCacheModule` | every stage | The Cache find, intel `cache`, the arrival sequence in Unknown Site (`onFrame`: inner voice, pings, music, the guardian of role `cacheGuardian` with `FleetOrders.defendSystem`, motes) and the command core, which `KestevenQuest.showCacheCore` claims and continues from the guardian's encounter (`# KESTEVEN QUESTLINE: CACHE`) |
| `KestevenHintWreckModule` | every stage | The job 4 hint wreck's claim (trigger `nskr_kqHintWreck`, placed by `KestevenJob4Module`) and action `readHintWreck`, so the wreck can be read after job 4 (hint wreck rows of `# KESTEVEN QUESTLINE: JOB 4`) |

The intel classes outside the quest framework (`ContractIntel`, `HellSpawnIntel`, `HellSpawnJudgementIntel`, `ThronesGiftIntel`) add themselves as scripts and put their per-frame logic in `advanceImpl()`. Vanilla `BaseIntelPlugin.advance()` runs the `endAfterDelay()` countdown, stops calling `advanceImpl()` once the intel is ending and reports the script done once it has ended; overriding `advance()` disables all of that.
...
| `starts/thronesgift/ThronesGiftManager`, `ThronesGiftIntel`, `AutomateDialog` | XP-to-automation points, the ship list and the automation itself (`getAutomatableShips`, `getAutomationCost`, `automate`). The intel's button opens `AutomateDialog`, a Java `InteractionDialogPlugin` shown with a null target, which prints the ship picker's result, the confirmation and the receipts, and redraws the intel panel with `IntelUIAPI.recreateIntelUI()` when the player leaves, because dismissing the dialog does not. |
