...
| Quest entity opens the wrong dialog | `CorePlugin.pickInteractionDialogPlugin`: claimed entities (`quest/QuestDialogs.CLAIM_KEY`, such as the blacksites) -> rules dialog on the claimed trigger; `quests/*Dialog`, custom-start FIDs |
...
| Named bounties | Quest `bounty` in `bounties/BountiesQuest` (module `quest/modules/BountyEncounter`, rows `# BOUNTY QUEST`); Mothership planets from `bounties/HeliosSite`; hints in `events/hints/HintManager`; [bounty structure](quests/CONTRACTS_AND_BOUNTIES.md#named-bounties) |
