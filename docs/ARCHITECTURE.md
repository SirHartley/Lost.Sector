...
| Endings and the production chip | `kesteven/quest/EndingKestevenDialog`, `kesteven/quest/EndingElizaDialog`, `# KESTEVEN QUESTLINE: ALTERNATIVE ENDINGS` rows `-> kesteven/quest/KestevenAltEndingsModule -> QuestHelper.saveEnding()`; `kesteven/quest/UnlimitedProductionChipCondition -> kesteven/BlackOpsManager.getUPC()` |
