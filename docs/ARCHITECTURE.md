...
| Endings and the production chip | `rules.csv` `# KESTEVEN QUESTLINE: ENDINGS` rows `-> kesteven/quest/KestevenEndingsModule`, `# KESTEVEN QUESTLINE: ALTERNATIVE ENDINGS` rows `-> kesteven/quest/KestevenAltEndingsModule -> QuestHelper.saveEnding()`; `kesteven/quest/UnlimitedProductionChipCondition -> kesteven/BlackOpsManager.getUPC()` |
...
| `IS_IRONSHELL` (`timid_xiv`) | `kesteven/quest/KestevenEndingsModule`, `QuestStageManager` |
...
| `kesteven/quest/QuestStageManager` | No quest logic left: reads and saves back the legacy fleet list `$kQuestMissionFleets`, which nothing fills. Runs while paused; the state exists from the end of `onGameLoad`. |
