...
|---|---|
| Kesteven questline stage or job | Conversation hub: `rules.csv` `# KESTEVEN QUESTLINE` rows `-> nskr_quest kq -> kesteven/quest/KestevenHubModule` checks ([dialogue map](quests/KESTEVEN_DIALOGUE.md#jack-alice-and-nicholas)); briefings and hand-ins: `rules.csv -> dialogue/rules/nskr_kestevenQuest -> kesteven/quest/QuestHelper.getStage/setStage -> quest/QuestManager` (quest `kq`, state `kesteven/quest/KestevenState`); job 1 world logic and intel in `kesteven/quest/KestevenJob1Module`, job 3 intel and acceptance setup in `KestevenJob3Module`, with `nskr_kqIntel*` rows in `# KESTEVEN QUESTLINE: JOB 1` and `JOB 3`; other automatic transitions in `kesteven/quest/QuestStageManager.advance()`; other intel `kesteven/quest/*Intel`; [questline walkthrough](quests/KESTEVEN_QUESTLINE.md), [saved state](quests/KESTEVEN_STATE.md) |
| Quest entity opens the wrong dialog | `CorePlugin.pickInteractionDialogPlugin`: claimed entities (`quest/QuestDialogs.CLAIM_KEY`, such as the blacksites) -> rules dialog on the claimed trigger; `quests/*Dialog`, `bounties/mothership/MothershipInteractionBlocker`, custom-start FIDs |
...
| Named bounties | Abyss and Eternity: quest `bounty` in `bounties/BountiesQuest` (module `quest/modules/BountyEncounter`, rows `# BOUNTY QUEST`); Mothership and Peacekeepers: `bounties/*/*Spawner -> intel/*Intel -> bounties/BountyLoot`; hints in `events/hints/HintManager`; [bounty structure](quests/CONTRACTS_AND_BOUNTIES.md#named-bounties) |
...
| `campaign/bounties` | Quest `bounty` (`BountiesQuest`, `BountiesStage`, `BountiesState`, `BountiesFleets`) for Abyss and Eternity; `BountyLoot`; packages `mothership/` and `peacekeepers/` |
...
|---|---|
| `CorePlugin` | Transient campaign plugin; `pickInteractionDialogPlugin` routes claimed entities to rules dialogs, and quest, Mothership and custom-start entities to Java dialogs |
...
| `bounties/BountiesQuest`, `BountiesStage`, `BountiesState`, `BountiesFleets` | Record quest `bounty` in `quest/QuestCatalog`: one `quest/modules/BountyEncounter` per bounty (Abyss, Eternity); fleets through `QuestFleets`, intel through `QuestIntels`; queries `location`, `sighted` and `carriesAbyssShips` for `HintManager` and `InterceptsQuest`. [Named bounties](quests/CONTRACTS_AND_BOUNTIES.md#named-bounties) |
| `bounties/mothership/MothershipSpawner`, `bounties/peacekeepers/RorqualSpawner` | One named bounty fleet each, spawned once per game; loot flag `LOOT_KEY`; `MothershipSpawner` also places the planets and `MothershipInteractionBlocker` |
...
| `bounties/BountyLoot`, `enigma/EnigmaFleetLoot` | Encounter loot listeners for the Mothership and Peacekeepers bounties and for Enigma casualties |

### Kesteven questline, contracts and hints

| File | Owner / connection |
|---|---|
| `kesteven/quest/KestevenQuest`, `KestevenStage`, `KestevenFlag`, `KestevenState`, `KestevenHubModule`, `KestevenJob1Module`, `KestevenJob3Module` | Quest `kq` in `quest/QuestCatalog`. `KestevenHubModule` is active in every stage and declares the checks, action and tokens of the conversation hub rows. `KestevenJob1Module` (`JOB1_ACTIVE`, `JOB1_DONE`) shows the job 1 intel entry (`QuestIntel` key `job1`), owns the tip system's dormant fleet (role `job1Dormant`) and moves the stage to `JOB1_DONE` after both deliveries. `KestevenJob3Module` (`JOB3_ACTIVE`, `JOB3_TARGET_KNOWN`) shows the job 3 intel entry (key `job3`) and places the bar event and the target's dormant fleet at acceptance. `KestevenState` holds every saved questline value; `QuestManager` is the only stage writer. Features outside the questline (`CorePlugin`, `ContractManager`, `ExileManager`, `StalkerSpawner`, `events/intercepts/InterceptsQuest`, `world/systems/cache/Cache`, `nskr_debt`, `nskr_shipSwap`, `nskr_modRemoval`, `nskr_barEventFixer`, the `nskr_isKStage` family) use only `KestevenQuest`'s public static [queries and actions](quests/KESTEVEN_STATE.md#queries-for-other-features). [State map](quests/KESTEVEN_STATE.md) |
...
| `kesteven/quest/*BarEvent` | Questline bar events. `kesteven/quest/KestevenJob3Module` adds `HostileTakeoverBarEvent` and `QuestStageManager` the Eliza search bar events to `PortsideBarData`; `dialogue/rules/nskr_barEventFixer` adds `DelveMeetingBarEvent` on each visit. |
| `kesteven/quest/*Intel` | Job intel: `OperationLifesaverIntel`, `TheDelveIntel`, `CacheIntel`. Jobs 1 and 3 use `QuestIntel` from `KestevenJob1Module` and `KestevenJob3Module`. |
