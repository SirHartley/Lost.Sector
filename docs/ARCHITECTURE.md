...
| `kesteven/quest/*BarEvent` | Questline Java bar events. `QuestStageManager` adds the Eliza search bar events to `PortsideBarData`; `dialogue/rules/nskr_barEventFixer` adds `DelveMeetingBarEvent` on each visit. |
