...
|---|---|---|
| Transient `EFS_LIST` | Built once per client session and reused for every save loaded in it: `HyperspaceEnigmaSpawner`, `HintManager`, `RorqualSpawner`, `EternitySpawner`, `enigma/HeartOccupation`, `StalkerSpawner`, `enigma/EnigmaRelations`, `KestevenScavenger`, `kesteven/KestevenExportManager`, `GuardSpawner`, `AbyssSpawner`, `QuestStageManager`, `kesteven/ExileManager`, `LoanShark`, `InterceptManager`, `BlackOpsManager`, `ContractManager`, `enigma/EnigmaHullmodListener`, `MothershipSpawner`, `BlacksiteManager`, `enigma/EnigmaAIConverter`, `GameModeManager`, `ThronesGiftManager`, `HellSpawnManager`; `HellSpawnNexListener` only with Nexerelin | Each is a `BaseCampaignEventListener` and `EveryFrameScript`, advanced every campaign frame. Removed before each save and re-added after it. Only `persistence/Saved` fields are reloaded per save; other instance fields carry over from the previous save loaded in the session. |
...
| `data/campaign/abilities.csv` | `nskr_hellSpawnAbility` -> `campaign/starts/hellspawn/HellSpawnAbility` |
...
| `starts/*/*Background` | Nexerelin backgrounds unlocked by the completed-story flags |
