...
public class EnigmaHullmodListener extends BaseCampaignEventListener implements EveryFrameScript {
    ...
    static void log(final String message) {
        Global.getLogger(EnigmaHullmodListener.class).info(message);
    }

    public EnigmaHullmodListener() {
        super(false);
        //init randoms
        getRandom();
    }
    ...
    public static void update() {
        ...
        while (count >= REQUIRED_PER_UNLOCK){
            ...
            if (count >= REQUIRED_PER_UNLOCK) {
                String key = DEFAULT_KEYS.get(MathUtilLS.getSeededRandomNumberInRange(0, DEFAULT_KEYS.size() - 1, random));
                //pick until new one
                while (unlocks.contains(key)) {
                    key = DEFAULT_KEYS.get(MathUtilLS.getSeededRandomNumberInRange(0, DEFAULT_KEYS.size() - 1, random));
                }
                //add
                unlocks.add(key);
    ...
    public void reportPlayerEngagement(EngagementResultAPI result) {
        ...
        if (lost.size()>0){
            for (FleetMemberAPI member : player.getDeployed()){
                ...
                if (!MiscLS.isProtTech(member)) continue;
                String type = MiscLS.protOrEnigma(member);
