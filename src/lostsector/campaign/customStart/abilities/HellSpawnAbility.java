...
import lostsector.util.CampaignBlastSpriteCreator;
...
public class HellSpawnAbility extends BaseDurationAbility {
    ...
    private void spawnHellFleets(){
        ...
        int num = MathUtilLS.getSeededRandomNumberInRange(MIN_FLEETS, MAX_FLEETS, random);
        for (int x = 0; x<num;x++) {
            ...
            simpleFleet.name = "Swarm" + " " + MiscLS.getRandomGreekLetter(random, true);
            ...
            //spawning
            final Vector2f loc = new Vector2f(MathUtils.getPointOnCircumference(pf.getLocation(), MathUtilLS.getSeededRandomNumberInRange(100f, 400f, random), random.nextFloat() * 360.0f));
