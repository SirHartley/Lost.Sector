package lostsector.campaign.fleets.bounties;
...
import lostsector.campaign.intel.MothershipIntel;
...
public class MothershipSpawner  extends BaseCampaignEventListener implements EveryFrameScript {
    //
    //spawns
    //
    ...
    static void log(final String message) {
        Global.getLogger(MothershipSpawner.class).info(message);
    }

    public MothershipSpawner() {
        super(false);
        //how often we run logic
        this.counter = new Saved<>(SAVED_PREFIX + "Counter", 0.0f);
        //for intel
        ...
        if (counter.val > 4f) {
            for (FleetInfo f : fleets) {
                ...
                if (visibleToPlayer && !despawn && firstTime.val) {
                    //Adds our intel
                    Global.getSector().getIntelManager().addIntel(new MothershipIntel(fleet), false);
                    log("added INTEL");
    ...
    public static void spawnPlanets(SectorEntityToken loc, Random random) {
        ...
        PlanetAPI a1 = system.addPlanet(PLANET1_ID, loc, "Helios", getRandomHabitableType(random),
                ...
                MathUtilLS.getSeededRandomNumberInRange(30f,50f, random),
                loc.getRadius() + MathUtilLS.getSeededRandomNumberInRange(200f,300f, random),
                MathUtilLS.getSeededRandomNumberInRange(90f,120f, random));
        ...
        PlanetAPI a2 = system.addPlanet(PLANET2_ID, loc, "Polaris", getRandomHabitableType(random),
                random.nextFloat()*360f,
                MathUtilLS.getSeededRandomNumberInRange(30f,50f, random),
                loc.getRadius() + MathUtilLS.getSeededRandomNumberInRange(400f,600f, random),
                MathUtilLS.getSeededRandomNumberInRange(120f,150f, random));
    ...
    private static String getRandomHabitableType(Random random) {
        return TYPES.get(MathUtilLS.getSeededRandomNumberInRange(0, TYPES.size()-1, random));
    }
    ...
    private static SectorEntityToken randomGasGiant(Random random) {
        ...
        return giants.get(MathUtilLS.getSeededRandomNumberInRange(0,giants.size()-1, random));
    }
    ...
    private static StarSystemAPI getRandomSystemWithBlacklist(Random random) {
        ...
        while (validSystems.isEmpty()){
            systems.add(MiscLS.getRandomNonCoreSystem(random));
            getSystemsWithGasGiant(systems, validSystems);
            log("ERROR no gas giant Remnant systems");
        }

        return validSystems.get(MathUtilLS.getSeededRandomNumberInRange(0,validSystems.size()-1, random));
    }
        ...
        public FleetInteractionDialogPluginImpl.FIDConfig createConfig() {
            ...
            config.delegate = new FleetInteractionDialogPluginImpl.BaseFIDDelegate() {
                ...
                public void notifyLeave(InteractionDialogAPI dialog) {
                    ...
                    FleetInfo info = null;
                    for (FleetInfo f : FleetUtil.getFleets(MothershipSpawner.FLEET_ARRAY_KEY)){
                        info = f;
                    }
                    if (info==null){
                        dialog.dismiss();
