...
import lostsector.campaign.intel.AbyssIntel;
...
public class AbyssSpawner extends BaseCampaignEventListener implements EveryFrameScript, ShipRecoveryListener {
    //
    ...
    static void log(final String message) {
        Global.getLogger(AbyssSpawner.class).info(message);
    }
        ...
        if (counter.val>4f) {
            for (FleetInfo f : fleets) {
                ...
                if(visibleToPlayer && firstTime.val){
                    //Adds our intel
                    AbyssIntel intel = new AbyssIntel(fleet);
    ...
    void spawnAbyssFleet(SectorEntityToken loc) {
        ...
        simpleFleet.name = FLEET_NAME+" "+ MiscLS.getRandomGreekLetter(random, true);
