...
import lostsector.campaign.intel.UmbraIntel;
...
public class EternitySpawner extends BaseCampaignEventListener implements EveryFrameScript, ShipRecoveryListener {
    //
    ...
    static void log(final String message) {
        Global.getLogger(EternitySpawner.class).info(message);
    }

    public EternitySpawner() {
        super(false);
        ...
        if (counter.val>4f) {
            for (FleetInfo f : fleets) {
                ...
                if(visibleToPlayer && firstTime.val){
                    //Adds our intel
                    UmbraIntel intel = new UmbraIntel(fleet);
