...
import lostsector.campaign.intel.RorqIntel;
...
public class RorqSpawner extends BaseCampaignEventListener implements EveryFrameScript {
    //
    ...
    static void log(final String message) {
        Global.getLogger(RorqSpawner.class).info(message);
    }
    ...
    public RorqSpawner() {
        ...
        this.newTargetCounter = new Saved<>(SAVED_PREFIX + "newTargetCounter", 0.0f);
        this.newGame = new Saved<>(SAVED_PREFIX + "NewGame", true);
        //for intel
        ...
        if (counter.val>10f) {
            for (FleetInfo f : fleets) {
                ...
                if (fleet.getFlagship()==null || fleet.getFlagship() != fleet.getFlagship()) {
                    fleet.getMemoryWithoutUpdate().unset(RorqSpawner.LOOT_KEY);
                }
                ...
                if (visibleToPlayer && firstTime.val) {
                    //Adds our intel
                    RorqIntel intel = new RorqIntel(fleet);
    ...
    private static CampaignFleetAPI reinforceFleet(CampaignFleetAPI fleet, float targetStrength){
        while (fleet.getFleetPoints()<(int)targetStrength){
            ...
            if (!member.isFighterWing()) {
                addSmods(member, MathUtilLS.getSeededRandomNumberInRange(1,3, getRandom()), fleet.getCommanderStats());
    ...
    private static PersonAPI addCaptain(int maxLevel){
        ...
        int level = MathUtilLS.getSeededRandomNumberInRange(maxLevel-2, maxLevel, getRandom());
        Map<String, Integer> skills = MiscLS.createRandomSkills(level, 0.67f, getRandom());
