...
public class BlacksiteManager extends BaseCampaignEventListener implements EveryFrameScript {
    ...
    static void log(final String message) {
        Global.getLogger(BlacksiteManager.class).info(message);
    }

    public BlacksiteManager() {
        super(false);
        //how often we run logic
        this.counter = new Saved<>("blacksiteManagerCounter", 0.0f);
    ...
    private void activateLoot(BlacksiteInfo site) {
        ...
        switch (site.faction){
            case Factions.PIRATES:
                entity = MiscLS.swapSalvageEntity(site.entity, PIRATE_ENTITY_ID, getRandom());
                break;
            case Factions.LUDDIC_PATH:
                entity = MiscLS.swapSalvageEntity(site.entity, PATHER_ENTITY_ID, getRandom());
                break;
            case Factions.TRITACHYON:
            ...
            case LSIds.ENIGMA_FACTION_ID:
                ...
                break;
            case Factions.REMNANTS:
                entity = MiscLS.swapSalvageEntity(site.entity, REMNANT_ENTITY_ID, getRandom());
                break;
        }
        if (entity==null) {
        ...
        for (int x = 0; x<count;x++) {
            ...
            switch (site.faction){
                ...
                case LSIds.ENIGMA_FACTION_ID:
                    ...
                    break;
                case Factions.REMNANTS:
                    simpleFleet.name = "Sub-Ordo "+ MiscLS.getRandomGreekLetter(random, true);
    ...
    private static SectorEntityToken getFleetLocation(StarSystemAPI sys, Random random){
        ...
        for (int x = 0; x<500;x++) {
            temp = MiscLS.getRandomLocationInSystem(sys, false, true, random);
