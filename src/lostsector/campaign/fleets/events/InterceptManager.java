...
public class InterceptManager extends BaseCampaignEventListener implements EveryFrameScript  {
    ...
    public InterceptManager() {
        super(false);
        //logic timer
        this.counter = new Saved<>("nskr_interceptManagerTimer", 0f);
        this.fleetCounter = new Saved<>("nskr_interceptManagerFleetTimer", 0f);
        //spawn check booleans
        this.aroFleetSpawned = new Saved<>("nskr_interceptManagerAROFleetSpawned", false);
        this.messengerFleetSpawned = new Saved<>("nskr_interceptManagerMessengerFleetSpawned", false);
        this.autoHunterFleetSpawned = new Saved<>("nskr_interceptManagerAutoHunterFleetSpawned", false);
        //init randoms
    ...
    static void log(final String message) {
        Global.getLogger(InterceptManager.class).info(message);
    }
    ...
    public void advance(float amount) {
        ...
        if (counter.val>10f){
            ...
            if (!aroFleetSpawned.val){
                //spawn check
                if (AbyssSpawner.hasBountyShips(pf) && hyperSpace && distance < IN_CORE_DIST) {
                    //rng check
    ...
    public void spawnAROfleet(Random random) {
        ...
        float combatPoints = MathUtilLS.getSeededRandomNumberInRange(110f, 130f, random);
        //power scaling
    ...
    public void spawnMessengerFleet(Random random) {
        ...
        float combatPoints = MathUtilLS.getSeededRandomNumberInRange(50f, 70f, random);
        //power scaling
        combatPoints += combatPoints* PowerLevel.get(0.2f, 0f,2f);
    ...
    public void spawnAutoHunterFleet(Random random) {
        ...
        float combatPoints = MathUtilLS.getSeededRandomNumberInRange(70f, 80f, random);
        //power scaling
        ...
        simpleFleet.ignoreMarketFleetSizeMult = true;
        simpleFleet.qualityOverride = MathUtilLS.getSeededRandomNumberInRange(0.70f, 0.80f, random);
        simpleFleet.sMods = MathUtilLS.getSeededRandomNumberInRange(1, 2, random);
        simpleFleet.assignment = FleetAssignment.INTERCEPT;
        simpleFleet.assignmentText = "intercepting your fleet";
        simpleFleet.interceptPlayer = true;
        CampaignFleetAPI fleet = simpleFleet.create();
