...
public class BlackOpsManager extends BaseCampaignEventListener implements EveryFrameScript {
    //
    ...
    void spawnOpsFleet(SectorEntityToken loc, SectorEntityToken target, String faction) {
        ...
        float points = MathUtilLS.getSeededRandomNumberInRange(50f, 150f, random);
        ...
        simpleFleet.sMods = MathUtilLS.getSeededRandomNumberInRange(2, 4, random);
        simpleFleet.name = FLEET_NAMES.get(MathUtilLS.getSeededRandomNumberInRange(0, FLEET_NAMES.size() - 1, random));
    ...
    public static SectorEntityToken createOpsTarget(SectorEntityToken home, FactionAPI faction){
        ...
        if (random.nextFloat()<DEFEND_CHANCE){
            ...
            //pick
            target = defend.get(MathUtilLS.getSeededRandomNumberInRange(0, defend.size() - 1, random));
        }
        //attack
        else {
            //has hostiles check
            if (hasHostileFactions(faction)){
                while (target==null){
                    ...
                    //pick
                    if (!attack.isEmpty()) target=attack.get(MathUtilLS.getSeededRandomNumberInRange(0, attack.size() - 1, random));
                }
            } else {
                //no hostiles
                //pick
                target= MiscLS.getRandomMarket(random, true);
            }
        }
