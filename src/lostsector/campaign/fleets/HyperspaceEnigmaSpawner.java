package lostsector.campaign.fleets;
...
import static lostsector.campaign.fleets.HyperspaceEnigmaSpawner.fleetLevel.*;
...
public class HyperspaceEnigmaSpawner extends BaseCampaignEventListener implements EveryFrameScript {
    ...
    static void log(final String message) {
        Global.getLogger(HyperspaceEnigmaSpawner.class).info(message);
    }

    public HyperspaceEnigmaSpawner() {
        super(false);
        //how often we run logic
        this.counter = new Saved<>("nskr_hyperspaceEnigmaSpawnerCounter", 0.0f);
        this.fleetCounter = new Saved<>("nskr_hyperspaceEnigmaSpawnerFleetCounter", 0.0f);
        //init randoms
    ...
    public void advance(float amount) {
        ...
        if (counter.val > 10f) {
            if (pf.isInHyperspace()) {
                ...
                float mult = MathUtilLS.normalize(dist, MIN_DISTANCE_FOR_SPAWNS, DISTANCE_FOR_MAX_SPAWNS);
                //max chance at max distance
                chance = MathUtilLS.lerp(0f, chance, mult);
                if (!MiscLS.enigmaExists()){
                    chance *= 0.25f;
                }
                ...
                if (FleetUtil.getFleets(FLEET_ARRAY_KEY).size() < MAX_FLEETS) {
                    float threatMult = StalkerSpawner.getEnigmaThreatScaling(false);
                    //rng check
                    if (getRandom(PERSISTENT_RANDOM_KEY).nextFloat() < chance * threatMult) {
                        Vector2f loc = new Vector2f(MathUtils.getPointOnCircumference(pf.getLocation(),
                                (pf.getSensorStrength() * 4.00f) * MathUtilLS.getSeededRandomNumberInRange(0.8f, 1.2f, getRandom(PERSISTENT_RANDOM_KEY)),
    ...
    void spawnEnigmaFleets(Vector2f loc, float power, taskType task) {
        ...
        float points = MathUtilLS.getSeededRandomNumberInRange(MIN_STRENGTH/2f, MAX_STRENGTH/2f, random);
        //
        points *= power;
        //
        float bias = 0.25f+(MathUtilLS.BiasFunction(random.nextDouble(), 0.33f));
        points *= bias;
        ...
        switch (task){
            case INTERCEPT:
                if (StalkerSpawner.getEnigmaThreatScaling(true)>0f) {
                    ...
                    simpleFleet.name = INTERCEPT_NAMES.get(MathUtilLS.getSeededRandomNumberInRange(0, INTERCEPT_NAMES.size() - 1, random));
                    ...
                    simpleFleet.qualityOverride = MathUtilLS.getSeededRandomNumberInRange(0.30f, 0.70f, random);
                    ...
                    simpleFleet.name = DORMANT_NAMES.get(MathUtilLS.getSeededRandomNumberInRange(0, DORMANT_NAMES.size() - 1, random));
                }
                break;
            case AMBUSH:
                keys.add(MemFlags.FLEET_IGNORES_OTHER_FLEETS);
                ...
                simpleFleet.sMods = MathUtilLS.getSeededRandomNumberInRange(0, 2, random);
                simpleFleet.assignmentText = "laying in wait";
                simpleFleet.noTransponder = true;
                simpleFleet.assignment = FleetAssignment.HOLD;
                simpleFleet.name = AMBUSH_NAMES.get(MathUtilLS.getSeededRandomNumberInRange(0, AMBUSH_NAMES.size() - 1, random));
                break;
            case DORMANT:
                keys.add(MemFlags.FLEET_IGNORES_OTHER_FLEETS);
                keys.add(MemFlags.FLEET_IGNORED_BY_OTHER_FLEETS);
            ...
            case GO_TO_SYSTEM:
                ...
                simpleFleet.sMods = MathUtilLS.getSeededRandomNumberInRange(0, 1, random);
                ...
                simpleFleet.name = MOVE_NAMES.get(MathUtilLS.getSeededRandomNumberInRange(0, MOVE_NAMES.size() - 1, random));
                break;
        }
    ...
    private SectorEntityToken getGoToTarget(Random random) {
        ...
        //pick
        StarSystemAPI system = validSystems.get(MathUtilLS.getSeededRandomNumberInRange(0, validSystems.size() - 1, random));
        //random loc
        return MiscLS.getRandomLocationInSystem(system, false, true, random);
    }
