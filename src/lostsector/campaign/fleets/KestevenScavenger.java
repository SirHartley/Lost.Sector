...
public class KestevenScavenger extends BaseCampaignEventListener implements EveryFrameScript {
    //
    ...
    static void log(final String message) {
        Global.getLogger(KestevenScavenger.class).info(message);
    }

    public KestevenScavenger() {
        super(false);
        //how often scavs spawn
        this.spawnCounter = new Saved<>("scavengerSpawnCounter", 0.0f);
        //how often we run logic
        this.counter = new Saved<>("scavengerCounter", 0.0f);
        ...
        if (counter.val>10f) {
            for (FleetInfo f : fleets) {
                ...
                if (playerVisible && Global.getSector().getPlayerFaction().getRelationship("kesteven") <= -0.15f) {
                    ...
                    rel = MathUtilLS.normalize(rel, 50f, 85f);
                    float extra = MathUtilLS.lerp(MAX_EXTRA_BETRAY_CHANCE, 0f, rel);
    ...
    void spawnScavFleets(MarketAPI market) {
        ...
        float bias = 0.25f+(MathUtilLS.BiasFunction(Math.random(), 0.75f));
        combatPoints *= bias;
