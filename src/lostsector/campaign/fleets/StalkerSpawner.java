...
import static lostsector.campaign.fleets.HyperspaceEnigmaSpawner.getFleetLevel;
...
public class StalkerSpawner extends BaseCampaignEventListener implements EveryFrameScript {
    ...
    Saved<Float> counter;
    Saved<SectorEntityToken> token;
    Saved<Boolean> questSpawn;
    Saved<LinkedList<StarSystemAPI>> locations;
    ...
    static void log(final String message) {
        Global.getLogger(StalkerSpawner.class).info(message);
    }
    public StalkerSpawner() {
        super(false);
        //how often stalkers spawn
        this.spawnCounter = new Saved<>("stalkerSpawnCounter", 0.0f);
        //how often we run logic
        this.counter = new Saved<>("stalkerCounter", 0.0f);
        this.locations = new Saved<>("stalkerLocations", new LinkedList<StarSystemAPI>());
        this.token = new Saved<>("stalkerToken", null);
        this.random = new Random();
        //quest
        this.questSpawn = new Saved<>("stalkerQuestSpawn", false);
        //init randoms
    ...
    public void advance(float amount) {
        ...
        if (locations.val.size() != 0) {
            ...
            StarSystemAPI defLoc = Global.getSector().getStarSystem(Frost.getName());
            target = defLoc;
        ...
        if (QuestUtil.getCompleted(GlacierCommsDialog.RECOVERED_KEY) && !questSpawn.val){
    ...
    public static float getEnigmaThreatScaling(boolean stalker) {
        ...
        protDP = MathUtilLS.normalize(protDP, 0f, 150f);
        protDP = MathUtilLS.lerp(0f,0.90f, protDP);
        //op
        protOP = Math.min(protOP, 250f);
        protOP = MathUtilLS.normalize(protOP, 0f, 250f);
        protOP = MathUtilLS.lerp(0f,0.40f, protOP);
        //apply
        scale += protDP+protOP;
    ...
    void spawnStalkerFleets(final Integer maxAmount, float power, Random random) {
        ...
        int amount = MathUtilLS.getSeededRandomNumberInRange(baseMinFleets, maxAmount, random);
        for (int x = 0; x < amount; x++) {
            float combatPoints = MathUtilLS.getSeededRandomNumberInRange(2, 14, random);
            ...
            if (random.nextFloat()<0.33f) simpleFleet.sMods = MathUtilLS.getSeededRandomNumberInRange(1,2, random);
            if (random.nextFloat()<0.15f) simpleFleet.qualityOverride = MathUtilLS.getSeededRandomNumberInRange(0.60f, 0.90f, random);
            if (random.nextFloat()<0.50f) simpleFleet.noTransponder = true;
            simpleFleet.name = edgyNames.get(MathUtilLS.getSeededRandomNumberInRange(0, edgyNames.size() - 1, random));
