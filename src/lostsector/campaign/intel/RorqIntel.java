...
public class RorqIntel extends BaseIntelPlugin {
    ...
    static void log(final String message) {
        Global.getLogger(RorqIntel.class).info(message);
    }

    //Initializer function
    public RorqIntel(CampaignFleetAPI fleet) {
        this.fleet = fleet;
    ...
    public void advance(float amount) {
        ...
        if (gone && BountyLoot.getPlayerDefeated(RorqSpawner.DEFEAT_ID)){
            payout = true;
            timer += amount;

            if (!doOnce && !Global.getSector().isPaused() && timer > 3f) {
                paid = Math.round(BountyLoot.getAmountPaid( RorqSpawner.DEFEAT_ID_PAID));
    ...
    protected void addBulletPoints(TooltipMakerAPI info, ListInfoMode mode) {
        ...
        if (!gone){
            ...
            info.addPara(Misc.getDGSCredits(RorqSpawner.BOUNTY_PAYOUT)+" reward", initPad, g, h, Misc.getDGSCredits(RorqSpawner.BOUNTY_PAYOUT), "");
        }
    ...
    public void createSmallDescription(TooltipMakerAPI info, float width, float height) {
        ...
        paid = Math.round(BountyLoot.getAmountPaid( RorqSpawner.DEFEAT_ID_PAID));
        String reducedPay = "";
        if (paid<(RorqSpawner.BOUNTY_PAYOUT-1f)){
        ...
        if (!gone){
            ...
            info.addPara("Shortly after, you were contact by an anonymous source telling you that there are many parties in the sector wishing to get rid of this fleet. Their payout is "+Misc.getDGSCredits(RorqSpawner.BOUNTY_PAYOUT), opad,tc,h,Misc.getDGSCredits(RorqSpawner.BOUNTY_PAYOUT)+"","");
        } if (payout && gone) {
