...
public class KQuest5Intel extends BaseIntelPlugin {
    ...
    static void log(final String message) {
        Global.getLogger(KQuest5Intel.class).info(message);
    }
    ...
    private void init(){
        ...
        elizaDialog =  QuestUtil.getCompleted(ElizaDialog.DIALOG_FINISHED_KEY);
        fightEliza =  QuestUtil.getCompleted(ElizaDialog.ELIZA_FIGHT_KEY);
        helpEliza =  QuestUtil.getCompleted(ElizaDialog.ELIZA_HELP_KEY);
        raidEliza = QuestUtil.getCompleted(ElizaDialog.ELIZA_RAID_KEY);
        killedEliza = QuestUtil.getCompleted(QuestStageManager.KILLED_ELIZA_KEY);
        agreeEliza = QuestUtil.getCompleted(ElizaDialog.AGREED_TO_HELP_KEY);
        ...
        recoveredGlacier = QuestUtil.getCompleted(GlacierCommsDialog.RECOVERED_KEY);
    ...
    protected void addBulletPoints(TooltipMakerAPI info, IntelInfoPlugin.ListInfoMode mode) {
        ...
        //satellites
        int satelliteCount = ArtifactDialog.getRecoveredSatelliteCount();
        if (stage == 16 && !aliceTip && satelliteCount<2) info.addPara("Recover disks from the comms satellites.", initPad, g, h, "", "");
        if (stage == 16 && aliceTip && !QuestUtil.getCompleted(ArtifactDialog.RECOVERED_3_KEY)) {
        ...
        if (stage == 16 && aliceTip && !QuestUtil.getCompleted(ArtifactDialog.RECOVERED_4_KEY)) {
        ...
        if (stage == 16 && aliceTip && jackTip && ArtifactDialog.getRecoveredSatelliteCount() >= 2 && !aliceTip2) {
        ...
        if (stage == 16 && aliceTip2 && !foundFrost) {
            ...
            float distLY = Misc.getDistanceLY(tipSystem.getConstellation().getLocation(), MiscLS.getFrost().getStar().getLocationInHyperspace())*1.5f;
            distLY *= 100f;
        ...
        if (stage == 16 && aliceTip2 && foundFrost && !recoveredGlacier) {
            info.addPara("Find the comms facility on the tundra planet in the " + MiscLS.getFrost().getName() + ".", initPad, g, h, MiscLS.getFrost().getName(), "");
        }
        ...
        if (stage == 16 && !foundEliza && paid && KQuest5ElizaBarMain.getDialogStage(KQuest5ElizaBarMain.INTRO_DIALOG_KEY)==2){
            String loc = KQuest5ElizaBarMain.getPaidForInfoTarget().getMarket().getName();
            String locSys = KQuest5ElizaBarMain.getPaidForInfoTarget().getStarSystem().getName();
