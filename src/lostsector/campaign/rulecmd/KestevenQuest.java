...
import lostsector.campaign.quests.GlacierCommsDialog;
...
public class KestevenQuest extends PaginatedOptions {
	...
	protected void addStageOptions()
	{
		...
		if (person==jack) {
			...
			if (stage == 16 && jackTip && aliceTip && !aliceTip2 && ArtifactDialog.getRecoveredSatelliteCount()>=2 && !allDisks) {
	...
	protected void showQuestInfoAndPrepare(TextPanelAPI text)
	{
		...
		if(stage==11) {
			...
			if (QuestUtil.outpostExists())text.addPara("\"Oh, by the way, you should talk to Nicholas Antoine. He works in communications and is currently stationed at "+ MiscLS.getOutpost().getName()+". He most likely has some more information.\"",tc,h, MiscLS.getOutpost().getName(),"");
		...
		if(stage==16 && person==jack && !jackTip) {
			boolean helpEliza = QuestUtil.getCompleted(ElizaDialog.ELIZA_HELP_KEY);
		...
		if(stage==16 && person==alice && !aliceTip) {
			int recovered = ArtifactDialog.getRecoveredSatelliteCount();
			...
			if (recovered==1 && QuestUtil.getCompleted(ArtifactDialog.RECOVERED_4_KEY)) {
			...
			if (recovered==1 && QuestUtil.getCompleted(ArtifactDialog.RECOVERED_3_KEY)) {
		...
		if(stage==16 && person==alice && jackTip && aliceTip && !aliceTip2) {
			StarSystemAPI frost = MiscLS.getFrost();
		...
		if(stage==16 && person==alice && jackTip && aliceTip && aliceTip2 && allDisks) {
			...
			if(QuestUtil.getCompleted(ElizaDialog.AGREED_TO_HELP_KEY))dialog.getOptionPanel().addOption("\"Yes\" (lie)", "nskr_kestevenQuestConfirmQuest"+"B");
			noLeave = true;
		}
	...
	protected void confirmSkip() {
		...
		if(stage == 7) {
			...
			MiscLS.getAlice().getRelToPlayer().adjustRelationship(-0.10f, RepLevel.VENGEFUL);
			//penalty text
			...
			MiscLS.addDormant(loc, "enigma", 45f, 50f, 0f, 1f, 1f, 1f, 1, 1);
		}
		text.setFontInsignia();
	}
	...
	public static void spawnEnvironmentalStorytelling(){
		...
		Frost.addDerelict(loc.getStarSystem(), "doom_Strike", MiscLS.createRandomNearOrbit(loc), ShipRecoverySpecial.ShipCondition.BATTERED, Math.random()<0.50f, null);
		Frost.addDerelict(loc.getStarSystem(), "atlas_Standard", MiscLS.createRandomNearOrbit(loc), ShipRecoverySpecial.ShipCondition.BATTERED, Math.random()<0.50f, null);
		Frost.addDerelict(loc.getStarSystem(), "shrike_Attack", MiscLS.createRandomNearOrbit(loc), ShipRecoverySpecial.ShipCondition.BATTERED, Math.random()<0.50f, null);
		...
		frost_main1.setOrbit(MiscLS.createRandomNearOrbit(loc));
	...
	protected void quest(){
		...
		if(stage == 2) {
			...
			Global.getSector().getFaction(Factions.PLAYER).adjustRelationship("kesteven",0.05f);
			MiscLS.getJack().getRelToPlayer().adjustRelationship(0.10f, RepLevel.COOPERATIVE);
			//completion text
			String payout = Misc.getDGSCredits(STAGE1_PAYOUT);
		...
		if(stage == 13 && !helped) {
			...
			MiscLS.getJack().setImportance(PersonImportance.HIGH);
		}
		//finish job 4 helped
		if(stage == 13 && helped) {
			text.setFontInsignia();
		...
		if(stage == 16 && jackTip && aliceTip && !allDisks) {
			...
			text.addPara("\"I think you are right, impressive. Now find the tundra planet in the "+ MiscLS.getFrost().getName()+" ASAP.\"",tc,h, MiscLS.getFrost().getName(),"");
	...
	private HullModSpecAPI getRewardMod() {
		...
		//new one check
		if (!tempMods.isEmpty()){
			return Global.getSettings().getHullModSpec(tempMods.get(MathUtilLS.getSeededRandomNumberInRange(0,tempMods.size()-1, getRandom())));
		} else {
			return Global.getSettings().getHullModSpec(MODS.get(MathUtilLS.getSeededRandomNumberInRange(0,MODS.size()-1, getRandom())));
		}
	}
