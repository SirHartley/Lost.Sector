package lostsector.campaign.quests.jobs;
...
public class ContractManager extends BaseCampaignEventListener implements EveryFrameScript {
    ...
    public ContractManager() {
        super(false);
        this.counter = new Saved<>("contractManagerCounter", 0.0f);
        this.resetCounter = new Saved<>("contractManagerResetCounter", 0.0f);

        //init randoms
        getRandom(Contracts.PERSISTENT_RANDOM_KEY_ELIMINATE);
        getRandom(Contracts.PERSISTENT_RANDOM_KEY_RECOVERY);
    }
    ...
    public void advance(float amount) {
        ...
        if (resetCounter.val>=600f){
            //reset
            if (Contracts.getContract(Contracts.CONTRACT_KEY_ELIMINATE)!=null){
                Contracts.setContract(Contracts.CONTRACT_KEY_ELIMINATE, null);
            }
            if (Contracts.getContract(Contracts.CONTRACT_KEY_RECOVERY)!=null){
                Contracts.setContract(Contracts.CONTRACT_KEY_RECOVERY, null);
            }
                ...
                if (enemy.getFleet().getFaction().isHostileTo(Global.getSector().getFaction(LSIds.KESTEVEN_FACTION_ID))) {
                    for (FleetMemberAPI m : lost) {
                        ...
                        switch (contract.subType) {
                            ...
                            case ("logistics"):
                                if (mspec.getHints() == null) continue;
                                if (MiscLS.isLogistics(mspec.getHints())) {
                                    completed++;
                                }
                                break;
                            case ("carrier"):
    ...
    public static boolean maxContracts(List<ContractInfo> contracts, ContractInfo.contractType type){
        ...
        int storyBonus = 1;
        if (QuestUtil.getCompleted(EndingKestevenDialog.DIALOG_FINISHED_KEY)){
            storyBonus = 2;
        }
