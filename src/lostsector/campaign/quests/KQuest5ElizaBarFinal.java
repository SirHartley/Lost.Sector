...
public class KQuest5ElizaBarFinal extends BaseBarEvent {
	...
	public KQuest5ElizaBarFinal() {
		seed = Misc.random.nextLong();
	}

	public boolean shouldShowAtMarket(MarketAPI market) {
		//used markets check
		if (KQuest5ElizaBarMain.getUsedMarkets(KQuest5ElizaBarMain.USED_MARKET_KEY).contains(market.getId())) return false;

		if (QuestUtil.getStage()<16) return false;
		paid = QuestUtil.getCompleted(KQuest5ElizaBarMain.PAID_FOR_INFO);
		if (paid && market!=KQuest5ElizaBarMain.getPaidForInfoTarget().getMarket())return false;
	...
	public void addPromptAndOption(InteractionDialogAPI dialog, Map<String, MemoryAPI> memoryMap) {
		...
		count = KQuest5ElizaBarMain.getDialogStage(KQuest5ElizaBarMain.INTRO_DIALOG_KEY);
	...
	public void optionSelected(String optionText, Object optionData) {
		...
		if (optionData== OptionId.A1){
			//set early
			if (count==2){
				List<String> markets = KQuest5ElizaBarMain.getUsedMarkets(KQuest5ElizaBarMain.USED_MARKET_KEY);
				markets.add(dialog.getInteractionTarget().getMarket().getId());
				KQuest5ElizaBarMain.setUsedMarkets(KQuest5ElizaBarMain.USED_MARKET_KEY, markets);
			}
			options.clearOptions();
		...
		if (optionData== OptionId.LEAVE){
			...
			if (count==2){
				...
				KQuest5ElizaBarMain.setDialogStage(3, KQuest5ElizaBarMain.INTRO_DIALOG_KEY);
			}
