...
public class QuestUtil {
    ...
    public static SectorEntityToken spawnArtifact(SectorEntityToken loc, int number) {
        ...
        artifact.setCircularOrbitPointingDown(loc, random.nextFloat()*360f, MathUtils.getDistance(artifact.getLocation(), loc.getLocation()), MathUtilLS.getSeededRandomNumberInRange(30f,60f, random));
    ...
    public static MarketAPI asteriaOrOutpost(){
        ...
        if (ExileManager.getExiled(ExileManager.EXILE_KEY)){
            market = MiscLS.getOutpost().getMarket();
        } else market = MiscLS.getAsteria().getMarket();
    ...
    public static StarSystemAPI getRandomSystemNearCore(Random random) {
        ...
        return MiscLS.getRandomNonCoreSystem(random);
    }
    ...
    public static SectorEntityToken pickElizaMarket(Random random, boolean ignoreUsedMarket) {
        ...
        for (MarketAPI market : Misc.getFactionMarkets(Factions.PIRATES)) {
            ...
            if (!ignoreUsedMarket) {
                if (KQuest5ElizaBarMain.getUsedMarkets(KQuest5ElizaBarMain.USED_MARKET_KEY).contains(market.getId())) {
                    isValid = false;
                }
            }
        ...
        if (validMarkets.isEmpty()){
            if (ignoreUsedMarket){
                ...
                return getRandomFactionMarket(random, Factions.PIRATES, KQuest5ElizaBarMain.getUsedMarkets(KQuest5ElizaBarMain.USED_MARKET_KEY));
            }
    ...
    public static SectorEntityToken getRandomFactionMarket(Random random, String faction, List<String> blacklist) {
        ...
        if (validMarkets.isEmpty()){
            if (!blacklist.isEmpty()){
                ...
                return MiscLS.getRandomMarket(random, false);
            }
        }
        return validMarkets.get(MathUtilLS.getSeededRandomNumberInRange(0,validMarkets.size()-1, random)).getPrimaryEntity();
    }
    ...
    public static SectorEntityToken spawnAwayFromStarFixer(SectorEntityToken entity, float extraDistanceMult){
        ...
        if (length>0f && length < toDistance) {
            while (length < toDistance) {
                Vector2f vector = new Vector2f(MathUtilLS.scaleVector(Vector2f.sub(entity.getLocation(), planet.getLocation(), entity.getLocation()), 1.25f));
