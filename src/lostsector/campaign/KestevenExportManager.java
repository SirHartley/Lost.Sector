...
public class KestevenExportManager extends BaseCampaignEventListener implements EveryFrameScript  {
    ...
    static void log(final String message) {
        Global.getLogger(KestevenExportManager.class).info(message);
    }

    public KestevenExportManager() {
        super(false);
        this.counter = new Saved<>("exportCounter", 0.0f);
    }
    ...
    public void advance(float amount) {
        ...
        if (counter.val>TIMER*10f) {
            ...
            for (FactionAPI f : allFactions)
            {
                ...
                if (level==ExportLevel.NONE || !MiscLS.kestevenExists()) {
    ...
    public static int getSavedProduction(){
        ...
        if (!data.containsKey(LicensingFees.TOTAL_KEY))
            data.put(LicensingFees.TOTAL_KEY, 0);

        return (int)data.get(LicensingFees.TOTAL_KEY);
    }

    public static void saveProduction(int amount){
        Map<String, Object> data = Global.getSector().getPersistentData();
        data.put(LicensingFees.TOTAL_KEY, amount);
    }
