...
public class CombatUtilsLS {

    static void log(final String message) {
        Global.getLogger(CombatUtilsLS.class).info(message);
    }
    ...
    private static void applyAOE(ShipAPI source, Vector2f point, float baseDamage, DamageType type, float expRange, CombatEntityAPI c) {
        ...
        damage *= MathUtilLS.smoothStep(1f- MathUtilLS.normalize(range,0f, expRange));
        if (damage<=0f) return;
        if (c instanceof ShipAPI){
            if ((c.getShield() != null && c.getShield().isOn() && c.getShield().isWithinArc(
                ...
                damageB *= 1f - MathUtilLS.smoothStep(MathUtilLS.normalize(rangeB, 0f, expRange));
