...
public class PowerLevel {
    ...
    public static float get(float base, float minPower, float maxPower){
        ...
        for (FleetMemberAPI ship : fleet.getFleetData().getMembersListCopy()){
            ...
            float wingValue = 0f;
            boolean unfit = false;
            float dMods = MiscLS.getDMods(ship.getVariant());
            ...
            if (MiscLS.isLogistics(ship.getHullSpec().getHints())){
                value = 0f;
        ...
        totalValue = MathUtilLS.normalize(totalValue, 0f, 3000000f);
        totalValue = MathUtilLS.lerp(0.20f, 1.0f, totalValue);
        power *= totalValue;
        //log("PowerLevel " + " totalValue " + totalValue);
        //level
        levelValue = MathUtilLS.normalize(stats.getLevel(), 1f, 15f);
        levelValue = MathUtilLS.lerp(0.25f, 1.0f, levelValue);
        power *= levelValue;
        //log("PowerLevel " + " levelValue " + levelValue);
        //officers
        officerValue = Math.min(50f, officerValue);
        officerValue = MathUtilLS.normalize(officerValue, 0f, 50f);
        officerValue = MathUtilLS.lerp(0.60f, 1.0f, officerValue);
        power *= officerValue;
        //log("PowerLevel " + " officerValue " + officerValue);

        power = MathUtilLS.lerp(0f, maxPower, power);
        power += base;
