...
public class KaboomPlugin extends BaseEveryFrameCombatPlugin {
    ...
    static void log(final String message) {
        Global.getLogger(KaboomPlugin.class).info(message);
    }
    ...
    void kaboom(Aed.ShipSpecificData data, ShipAPI ship) {
        CombatEngineAPI engine = Global.getCombatEngine();
        if (Global.getCombatEngine().isPaused()) {
            return;
        }
        ...
        if (data.timer < 2.5f) {
            ...
            //nebula
            if (!data.doOnce) {
                Vector2f vel = MathUtilLS.scaleVector(ship.getVelocity(), 0.25f);
                for (int x = 0; x < 10; x++) {
            ...
            for (int x = 0; x < 4; x++) {
                ...
                particlePos = MathUtils.getRandomPointOnCircumference(data.kLoc, MathUtilLS.lerp(200, 600, timeOffset));
                particleVel = Vector2f.sub(particlePos, data.kLoc, null);
                float targetLength = MathUtilLS.lerp(600, 0, timeOffset);
                ...
                color = new Color(255, 31, 57, (int) MathUtilLS.lerp(200, 150, timeOffset));
