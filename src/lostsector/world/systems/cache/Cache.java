...
public class Cache {
    ...
    static void log(final String message) {
        Global.getLogger(Cache.class).info(message);
    }
    ...
    public static void spawnEverything(StarSystemAPI system){
        ...
        satellite1.getMemory().set(ArtifactDialog.ARTIFACT_EMPTY_KEY, true);
        ...
        satellite2.getMemory().set(ArtifactDialog.ARTIFACT_EMPTY_KEY, true);
        satellite2.setCircularOrbitPointingDown(center, 180, 400, 60f);

        //DORMANT
        for (int x = 0; x < 2; x++) {
            SectorEntityToken dormantFleet = MiscLS.addDormant(center, Factions.DERELICT, 125f, 175f, 1.00f, 0.40f, 0.60f, 0f, 0, 0);
    ...
    public static CampaignFleetAPI spawnGuardianFleet(CampaignFleetAPI pf, SectorEntityToken loc) {
        ...
        for (FleetMemberAPI m : fleet.getMembersWithFightersCopy()){
            if (m.isFighterWing() && MiscLS.isProtTech(m)){
        ...
        public FleetInteractionDialogPluginImpl.FIDConfig createConfig() {
            ...
            config.delegate = new FleetInteractionDialogPluginImpl.BaseFIDDelegate() {
                ...
                public void notifyLeave(InteractionDialogAPI dialog) {
                    ...
                    InteractionDialogPlugin plugin = new CoreDialog();
    ...
    public static boolean hasPrototypes(CampaignFleetAPI fleet){
        ...
        for (FleetMemberAPI m : fleet.getMembersWithFightersCopy()) {
            if (MiscLS.isProtTech(m))hasProt = true;
            if (hasProt)break;
        }
        return hasProt;
    }
    ...
    private static Vector2f createCacheLoc(float minDistanceFromCore, float maxDistanceFromCore, float maxDistanceFromNearestSystem){
        ...
        for (int x=0;x<250;x++) {
            for (StarSystemAPI system : Global.getSector().getStarSystems()) {
                ...
                if (loc.length()>minDistanceFromCore && loc.length()<maxDistanceFromCore && MiscLS.getDistanceFromNearestSystem(loc)<maxDistanceFromNearestSystem){
                    done=true;
    ...
    public static PersonAPI createSecondaryCaptain(int number) {
        ...
        Map<String, Integer> skills = new HashMap<>(OFFICER_SKILLS);
        if (ModPlugin.getStarfarerMode()) skills.put(Skills.POLARIZED_ARMOR,2);
        MiscLS.setOfficerSkills(commander, skills);
