...
public class Inertial extends BaseHullMod {
    ...
    static void log(final String message) {
        Global.getLogger(Inertial.class).info(message);
    }
    ...
    public void advanceInCombat(ShipAPI ship, float amount) {
        ...
        if (data.velocity>0f){
            ...
            float dmgNormalized = MathUtilLS.normalize(vel, 0f , max);
            ...
            for (WeaponAPI wep : ship.getAllWeapons()){
                ...
                if (spec.getType() == WeaponAPI.WeaponType.BALLISTIC || spec.getType() == WeaponAPI.WeaponType.ENERGY){
                    ...
                    Color glow = MiscLS.setAlpha(GLOW_COLOR, (int)(180*dmgNormalized));
                    if (colorInterval.intervalElapsed()) {
                        glow = MiscLS.randomiseColor(glow, (int)(20*dmgNormalized), (int)(20*dmgNormalized), (int)(50*dmgNormalized), (int)(50*dmgNormalized), true);
                    }
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        if (ship!=null) {
            ...
            tooltip.addPara("", 0.0f, MiscLS.NICE_YELLOW, "");
            tooltip.addPara("Note there is no cap to this bonus, this is simply for reference.", 0.0f, MiscLS.NICE_YELLOW, "Note");

            if (Global.getCurrentState() == GameState.CAMPAIGN){
                if (ship.getHullSize()== ShipAPI.HullSize.FRIGATE) {
                    tooltip.addSectionHeading("S-mod penalty", bad, badbg, Alignment.MID, pad);
                    tooltip.addPara("Increases the damage taken by weapons by " + (int) SMOD_PENALTY + "%%.", pad, MiscLS.NICE_YELLOW, (int) SMOD_PENALTY + "%");
                    if (!sMod)
                ...
                if (ship.getHullSize()== ShipAPI.HullSize.CRUISER || ship.getHullSize()== ShipAPI.HullSize.CAPITAL_SHIP) {
                    ...
                    tooltip.addPara("Increases projectile velocity by " + (int) SMOD_BONUS + "%%.", pad, MiscLS.NICE_YELLOW, (int) SMOD_BONUS + "%");
                    if (!sMod)
                        tooltip.addPara("This effect only applies if this hullmod is built into the hull using a story point. Cheap hullmods have stronger effects.", pad, grg, "story point");
                }
            }
