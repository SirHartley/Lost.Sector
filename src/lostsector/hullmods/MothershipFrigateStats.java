...
public class MothershipFrigateStats extends BaseHullMod {

    static void log(final String message) {
        Global.getLogger(MothershipFrigateStats.class).info(message);
    }

    @Override
    public void applyEffectsBeforeShipCreation(ShipAPI.HullSize hullSize, MutableShipStatsAPI stats, String id) {

        stats.getMaxCombatReadiness().modifyFlat(id, Mothership.CR_BONUS * 0.01f, "Mothership Subroutine bonus");

        stats.getDynamic().getMod(Stats.DEPLOYMENT_POINTS_MOD).modifyFlat(Mothership.FUEL_BASE_KEY, -1f);
        stats.getSuppliesToRecover().modifyFlat(Mothership.FUEL_BASE_KEY, -1f);
