...
import lostsector.shipsystems.CausalityStats;
...
public class Causality extends BaseHullMod {
	...
	public void advanceInCombat(ShipAPI ship, float amount) {
		...
		if (data.energy>0){
			...
			if (Global.getCombatEngine().getPlayerShip() == ship) {
				PostProcessShader.setNoise(false, MathUtilLS.lerp(0f, 0.25f, effectSqrt));
				PostProcessShader.setLightness(false, MathUtilLS.lerp(1f, 1.25f, effectSqrt));
				PostProcessShader.setSaturation(false, MathUtilLS.lerp(1f, 0.5f, effectSqrt));
				...
				float max = 8000f;
				if (data.energy>min){
					float modSynth = MathUtilLS.normalize(MathUtils.clamp(data.energy, min, max), min, max);
					float mult = 1.5f;
					float modNoise = MathUtilLS.normalize(MathUtils.clamp(data.energy, min*mult, max*mult), min*mult, max*mult);
	...
	public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
		...
		text.addPara(TEXT1, 0.0f, MiscLS.NICE_YELLOW, TEXT1);
		text.addPara("-15%% weapon flux usage.", 2.0f, MiscLS.BON_GREEN, "15%");
		text.addPara("+15%% energy weapon damage.", 2.0f, MiscLS.BON_GREEN, "15%");
		text.addPara("+15%% ballistic weapon rate of fire (with an additional matching reduction in flux usage).", 2.0f, MiscLS.BON_GREEN, "15%");
		...
		text.addPara("-Absorbed projectiles grant more energy than missiles.", 2.0f, MiscLS.BON_GREEN, "");
		text.addPara("-Absorbed missiles deal 1x the original damage while absorbed projectiles deal 1.5x the damage.", 2.0f, MiscLS.BON_GREEN, "");
		text.addPara("-Created bolts counts as an energy weapon for bonuses, while created homing blobs count as a missile weapon for bonuses.", 2.0f, MiscLS.BON_GREEN, "");
		tooltip.addImageWithText(pad);

		text = tooltip.beginImageWithText("graphics/icons/hullsys/entropy_amplifier.png", 36.0f);
		text.addPara(TEXT3, 0.0f, MiscLS.TT_ORANGE, TEXT3);
		text.addPara("-Going above energy capacity is dangerous.", 2.0f, MiscLS.TT_ORANGE, "dangerous");
	...
	private class IndicatorRenderer implements CombatLayeredRenderingPlugin {
		...
		public void render(CombatEngineLayers layer, ViewportAPI viewport) {
			...
			targets.addAll(CombatUtilsLS.getProjectilesWithinRange(ship.getLocation(), range));
			targets.addAll(CombatUtilsLS.getMissilesWithinRange(ship.getLocation(), range));
			...
			for (Pair<CombatEntityAPI, Float> target : newTargets) {
				if (target.one instanceof DamagingProjectileAPI) {
					if (!CausalityStats.isValid(target.one,ship)) continue;
					...
					float sizeMult;
					float alpha = 1f;
					sizeMult = Math.max(1,1f+(MathUtilLS.smoothStep(MathUtilLS.lerp(0f, 1f, target.two))));
					if (target.two>0.67f){
						alpha = MathUtilLS.smoothStep(MathUtilLS.lerp(0f, 3f, MathUtilLS.inverse(target.two)));
					}
					Color color = COLOR1;
					color = MiscLS.setAlpha(color, (int)(color.getAlpha()*alpha));
