...
public class ProtExplosion {
    ...
    static void log(final String message) {
        Global.getLogger(ProtExplosion.class).info(message);
    }

    public static class ProtExplosionListener implements AdvanceableListener {

        public ShipAPI ship;

        public ProtExplosionListener(ShipAPI ship) {
            this.ship = ship;
        }

        public void advance(float amount) {
            //AED check
            Aed.ShipSpecificData data = (Aed.ShipSpecificData) Global.getCombatEngine().getCustomData().get("KABOOM_DATA_KEY" + ship.getId());
            if (data != null) {
            ...
            Color color = MiscLS.randomiseColor(PARTICLE_COLOR, 50, 25,0,25,false);
            for (int x = 0; x < 200*mag; x++) {
                ...
                Global.getCombatEngine().addSmokeParticle(particlePos, MathUtilLS.scaleVector(particleVel, 4f), 4f, 0.67f, 2.0f,
                        color);
            }
            //nebula fx
            Global.getCombatEngine().addNebulaParticle(
                    ship.getLocation(), MathUtilLS.scaleVector(ship.getVelocity(),0.5f), (ship.getCollisionRadius()+100f)*mag,
                    1.20f, 0.5f,1.5f,4f,
                    NEBULA_COLOR);
            ...
            for (int x = 0; x < 24*mag; x++) {
                ...
                empPosTo = MathUtils.getPointOnCircumference(ship.getLocation(), radius,
                        angle+ MathUtilLS.getRandomNumberInRangeExcludingRange(-70f, 70f,-30f,30f));
