...
public class Augmented extends BaseHullMod {
	...
	static {
		...
		STAT_HULLMODS.add(LSIds.HIGH_CAPACITANCE_BANKS_HULLMOD_ID);
	}
	...
	static {
		...
		DURABILITY_HULLMODS.add(LSIds.CRITICAL_POINT_PROTECTION_HULLMOD_ID);
	}

	static void log(final String message) {
		Global.getLogger(Augmented.class).info(message);
	}
    ...
    public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
		...
		if (stats.getVariant().hasHullMod(LSIds.CRITICAL_POINT_PROTECTION_HULLMOD_ID)) {
			stats.getMinArmorFraction().modifyPercent(LSIds.CRITICAL_POINT_PROTECTION_HULLMOD_ID +"_augment", CRITICAL_POINT_BONUS);
		}
	}
	...
	public void applyEffectsAfterShipCreation(ShipAPI ship, String id) {
		...
		if (stats.getVariant().hasHullMod(FLUX_COIL_ADJUNCT_ID)) {
			ship.addListener(new AugmentedListener.FluxCoilAdjunctListener(ship));
		}
		// BLAST DOORS
		if (stats.getVariant().hasHullMod(BLAST_DOORS_ID)) {
			ship.addListener(new AugmentedListener.BlastDoorsListener(ship));
		}
		// SAFETY OVERRIDE
		if (stats.getVariant().hasHullMod(SAFETY_OVERRIDE_ID)) {
			ship.addListener(new AugmentedListener.SafetyOverrideListener(ship));
		}
		// UNSTABLE INJECTOR
		if (stats.getVariant().hasHullMod(UNSTABLE_INJECTOR_ID)) {
			ship.addListener(new AugmentedListener.UnstableInjectorListener(ship));
		}
		// NAV RELAY
		if (stats.getVariant().hasHullMod(NAV_RELAY_ID)) {
			ship.addListener(new AugmentedListener.NavRelayListener(ship));
		}
		//OPERATIONS CENTER
		if (stats.getVariant().hasHullMod(OPERATIONS_CENTER_ID)) {
			ship.addListener(new AugmentedListener.OpCenterListener(ship));
		}
	}
	...
	public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
		...
		// BIG BATS
		if (ship.getVariant().hasHullMod(LSIds.HIGH_CAPACITANCE_BANKS_HULLMOD_ID)) {
			TooltipMakerAPI text = tooltip.beginImageWithText("graphics/icons/hullsys/high_energy_focus.png", 32.0f);
		...
		// CRITICAL POINT PROTECTION
		if (ship.getVariant().hasHullMod(LSIds.CRITICAL_POINT_PROTECTION_HULLMOD_ID)) {
			TooltipMakerAPI text = tooltip.beginImageWithText("graphics/icons/hullsys/damper_field.png", 32.0f);
