...
import lostsector.campaign.EnigmaHullmodListener;
import lostsector.util.MiscLS;

import java.awt.*;
import java.util.ArrayList;

public class Domain_era extends BaseHullMod {
	...
	public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
		...
		EnigmaHullmodListener.update();

		ArrayList<String> unlocks = EnigmaHullmodListener.getUnlocks(EnigmaHullmodListener.UNLOCKS_MEM_KEY);
		//everything unlocked
		if (unlocks.size() < EnigmaHullmodListener.DEFAULT_KEYS.size()){
		...
		if (unlocks.contains(EnigmaHullmodListener.KEY_BASE+"#1")) {
			tooltip.addPara("-Supply use for maintenance increased by "+"150"+"%"+"%", pad, bad, "150"+"%");
		}
		if (unlocks.contains(EnigmaHullmodListener.KEY_BASE+"#5")) {
		...
		if (unlocks.contains(EnigmaHullmodListener.KEY_BASE+"#2")) {
			tooltip.addPara("-Maneuverability is reduced by "+(int)(SPEED_PENALTY*2f)+"%"+"%", 2.0f, bad, (int)(SPEED_PENALTY*2f)+"%");
		}
		if (unlocks.contains(EnigmaHullmodListener.KEY_BASE+"#3")) {
		...
		if (unlocks.contains(EnigmaHullmodListener.KEY_BASE+"#7")) {
			tooltip.addPara("-Weapon and engine repair speed increased by "+(int)(REPAIR_BONUS)+"%"+"%", 2.0f, y, (int)(REPAIR_BONUS)+"%");
		}
		if (unlocks.contains(EnigmaHullmodListener.KEY_BASE+"#8")) {
		...
		if (unlocks.size()>=EnigmaHullmodListener.DEFAULT_KEYS.size()){
			...
			tooltip.addPara("Regenerates "+(int)Rage_mod.HULL_REPAIR_PERCENT+"%% of maximum hull and recover every armor cell to 75%% if below that value. " +
					"Dissipate 50%% of current flux, cruiser size hulls are phased for "+(int)Rage_mod.MAX_DISAPPEAR_TIME+" seconds. " +
					"Lose "+(int)Math.abs(Rage_mod.ENRAGE_PPT_PENALTY)+"%% of remaining peak performance time.", pad, y, "");
			tooltip.addPara("Gain major improvements to many stats", pad, y, "");

			tooltip.addPara("-Top speed is increased both by "+(int)(Rage_mod.ENRAGE_SPEED_BONUS)+"%% " +
					"and by an additional "+(int)(Rage_mod.ENRAGE_SPEED_BONUS_FLAT)+"su/s flat ", pad, y,  (int)(Rage_mod.ENRAGE_SPEED_BONUS)+"%", (int)(Rage_mod.ENRAGE_SPEED_BONUS_FLAT)+"su/s");
			tooltip.addPara("-Ballistic weapon rate of fire increased and flux use reduced by "+(int)(Rage_mod.ENRAGE_BALLISTIC_ROF_BONUS)+"%"+"%", 2.0f, y, (int)(Rage_mod.ENRAGE_BALLISTIC_ROF_BONUS)+"%");
			tooltip.addPara("-Energy weapon damage increased by "+(int)(Rage_mod.ENRAGE_ENERGY_DMG_BONUS)+"%"+"%", 2.0f, y, (int)(Rage_mod.ENRAGE_ENERGY_DMG_BONUS)+"%");
			tooltip.addPara("-Flux dissipation increased by "+(int)(Rage_mod.ENRAGE_FLUX_MULTI_BONUS)+"%"+"%", 2.0f, y, (int)(Rage_mod.ENRAGE_FLUX_MULTI_BONUS)+"%");
			tooltip.addPara("-Shield unfold and turn rate increased by "+(int)(Rage_mod.SHIELD_SPEED_BONUS)+"%"+"%", 2.0f, y, (int)(Rage_mod.SHIELD_SPEED_BONUS)+"%");
		}
