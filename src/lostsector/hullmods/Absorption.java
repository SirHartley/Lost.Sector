...
public class Absorption extends BaseHullMod {
    //
    ...
    public void advanceInCombat(ShipAPI ship, float amount) {
        ...
        if (data.timer>0f){
            ...
            float timerNorm = MathUtilLS.normalize(data.timer,0f, SYS_TIME);
            float bonus = MathUtilLS.lerp(0f, 1f, timerNorm);
            ...
            ship.getShield().setInnerColor(MiscLS.blendColors(baseColor, SHIELD_COLOR, bonus));

            data.resist = Math.abs(resist);

            data.timer -= amount;
            if (player){
                float gain = MathUtilLS.lerp(1f, 0f, timerNorm);
        ...
        if (data.absorbed>0f){
            ...
            float mult = MathUtilLS.normalize(absorbed,0f, MIN_ENERGY);
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        text.addPara("activating the system greatly boosts shield performance, with the bonus decaying over "+(int)SYS_TIME+" seconds.", 0.0f, MiscLS.NICE_YELLOW, (int)SYS_TIME+"");
        text.addPara("-"+(int)Math.abs(DMG_TAKEN)+"%"+"% shield damage taken.", 0.0f, MiscLS.BON_GREEN, (int)Math.abs(DMG_TAKEN)+"%");
        text.addPara("+"+(int)SHIELD_SIZE+" shield arc and instant unfolding.", 0.0f, MiscLS.BON_GREEN, (int)SHIELD_SIZE+"");
        tooltip.addImageWithText(pad);
        text.addPara("",pad);
        TooltipMakerAPI text2 = tooltip.beginImageWithText("graphics/icons/hullsys/ammo_feeder.png", 36.0f);
        text2.addPara("damage that is blocked by the system is absorbed as energy that decays over time at "+(int)ENERGY_PER_SECOND+" units per second.", 0.0f, MiscLS.NICE_YELLOW, (int)ENERGY_PER_SECOND+"");
        text2.addPara("+"+(int)BONUS_ROF_MULT+"%"+"% non missile weapon rate of fire while having more than "+(int)MIN_ENERGY+" energy stored. having less than that scales the bonus down to zero.", 0.0f, MiscLS.BON_GREEN, (int)BONUS_ROF_MULT+"%");
        tooltip.addImageWithText(pad);
