...
public class Ultracaliber extends BaseHullMod {
    //
    ...
    static void log(final String message) {
        Global.getLogger(Ultracaliber.class).info(message);
    }
    ...
    public void advanceInCombat(ShipAPI ship, float amount) {
        ...
        if (data.timer>0f){
            ...
            float bonus = MathUtilLS.lerp(0f, SYS_ROF_MULT, timerNorm);
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        text.addPara("+"+(int)DAMAGE_MULT+"%"+"% projectile weapon damage.", 0.0f, MiscLS.BON_GREEN, (int)DAMAGE_MULT+"%");
        text.addPara("+"+(int)WEAPON_HEALTH+"%"+"% to durability of all weapons.", 0.0f, MiscLS.BON_GREEN, (int)WEAPON_HEALTH+"%");
        text.addPara("+"+(int)SYS_ROF_MULT+"%"+"% weapon rate of fire decaying over five seconds after system use.", 0.0f, MiscLS.BON_GREEN, (int)SYS_ROF_MULT+"%");
        //text.addPara("", pad);
        text.addPara("-"+(int)ROF_MULT+"%"+"% weapon rate of fire.", 0.0f, MiscLS.TT_ORANGE, (int)ROF_MULT+"%");
        text.addPara("-weapon range past "+(int)MAX_RANGE+" units is reduced by one third." , 0.0f, MiscLS.TT_ORANGE, (int)MAX_RANGE+"");
