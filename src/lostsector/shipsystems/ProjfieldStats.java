...
public class ProjfieldStats extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        //jitter
        Color color1 = MiscLS.randomiseColor(COLOR, 10, 5, 15, 5, true);
        Color color2 = color1;
        ...
        color1 = MiscLS.setAlpha(color1, (int)(color1.getAlpha()*effectLevel));
        float vSize = size2.getX();
        float nTimer = 0f;
        nTimer = MathUtilLS.normalize(timer,0f,0.5f);

        //engine.addFloatingText(ship.getLocation(), "timer " + nTimer, 48f, Color.cyan, ship, 0.5f, 1.0f);
        vSize = MathUtilLS.lerp(vSize*0.10f, vSize*1.50f, nTimer);
        size2 = new Vector2f(vSize, vSize);
        alpha = MiscLS.clamp255((int) MathUtilLS.lerp(alpha*1.0f, alpha*0.3f, nTimer));
        color2 = MiscLS.setAlpha(color1,(int)alpha);
        ...
        for (CombatEntityAPI ent : CombatUtils.getEntitiesWithinRange(ship.getLocation(), range)){
            ...
            if (ent instanceof MissileAPI) {
                ...
                damage = MathUtilLS.normalize(damage, MIN_EFFECT_DMG, MAX_EFFECT_DMG);
                float mult = 0f;
                mult = MathUtilLS.lerp(1f, MAX_SLOWDOWN, damage);

                if (!isSame) {
                    affectedEntities.add(new Pair<>(ent, ogVelocity));
                    //engine.addFloatingText(ent.getLocation(), "ADDED", 24, Color.RED, null, 0.5f, 1.0f);
                }
                missile.getVelocity().set(MathUtilLS.scaleVector(ogVelocity, mult));
                //fx
            ...
            if (ent instanceof DamagingProjectileAPI) {
                ...
                damage = MathUtilLS.normalize(damage, MIN_EFFECT_DMG, MAX_EFFECT_DMG);
                float mult = 0f;
                mult = MathUtilLS.lerp(MAX_TURNRATE/3f, MAX_TURNRATE, damage);
                //direction check
            ...
            if (ent instanceof ShipAPI){
                ...
                target.getVelocity().set(MathUtilLS.scaleVector(ogVelocity, FIGHTER_SLOWDOWN));
                //fx
