...
import lostsector.shipsystems.OmniLoadStats;
...
public class OmniLoadAI implements ShipSystemAIScript {
    ...
    public void advance(float amount, Vector2f missileDangerDir, Vector2f collisionDangerDir, ShipAPI target) {
        ...
        if (timer.intervalElapsed()) {
            ...
            float fluxMod = MathUtilLS.normalize(Math.min(maxFlux, 1500f), 0f, 1500f);
            //fluxed
            decisionLevel -= MathUtilLS.lerp(0f,40f*fluxMod, ship.getFluxLevel());
            //high ammo
            ...
            //cooldown
            float norm = MathUtilLS.normalize(currCooldown, 0f, maxCooldown);
            decisionLevel += MathUtilLS.lerp(0f, cooldownBonus, norm);
            //ammo
            if (ammoBonus>0f) {
                norm = MathUtilLS.normalize(currAmmo, 0, maxAmmo);
                decisionLevel += MathUtilLS.lerp(ammoBonus, 0f, norm);
            }
            ...
            for (ShipAPI t : CombatUtils.getShipsWithinRange(ship.getLocation(), 500f)){
                ...
                decisionLevel += MathUtilLS.inverse(t.getHullLevel()) * 30f;
