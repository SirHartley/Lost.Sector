...
import lostsector.shipsystems.HarmonicsStats;
...
public class HarmonicsAI  implements ShipSystemAIScript {
    ...
    public void advance(float amount, Vector2f missileDangerDir, Vector2f collisionDangerDir, ShipAPI target) {
        ...
        if (timer.intervalElapsed()) {
            ...
            for (ShipAPI s : engine.getShips()){
                if (!HarmonicsStats.isValidTarget(ship,s)) continue;
                potential.add(s);
            }
    ...
    private void evaluate(ShipAPI source, List<ShipAPI> targets) {
        ...
        for (ShipAPI t : targets){
            ...
            for (ShipAPI enemy : CombatUtils.getShipsWithinRange(t.getLocation(), getAverageRange(t))){
                ...
                score += 25f * MiscLS.getLinearMod(enemy, 1.0f);
            }
