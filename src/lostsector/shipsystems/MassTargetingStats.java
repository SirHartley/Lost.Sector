...
public class MassTargetingStats extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        if (!ship.hasListenerOfClass(MassTargeting.MassTargetingListener.class)) ship.addListener(new MassTargeting.MassTargetingListener(ship));
        ...
        color1 = MiscLS.setAlpha(color1,(int)alpha);
        float vSize = size2.getX();
        float nTimer = 0f;
        nTimer = MathUtilLS.normalize(timer,0f,2f);

        //engine.addFloatingText(ship.getLocation(), "timer " + nTimer, 48f, Color.cyan, ship, 0.5f, 1.0f);
        vSize = MathUtilLS.lerp(vSize*0.15f, vSize*1.25f, nTimer);
