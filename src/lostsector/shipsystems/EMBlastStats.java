...
public class EMBlastStats extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        if (state == State.IN) {
            // SPEED
            ship.getMutableStats().getMaxSpeed().modifyMult(id, MathUtilLS.inverse(effectLevel/2.0f));
            // tank
            ...
            //make the AI scared
            for (ShipAPI potentialTarget : CombatUtilsLS.getShipsWithinRange(ship.getLocation(), 450f)){
        ...
        if (state == State.ACTIVE) {
            ...
            if (!explosions) {
                //AOE DAMAGE
                CombatUtilsLS.applyAOEDamage(ship, null, ship.getLocation(), EXPLOSION_DAMAGE_AMOUNT, DamageType.ENERGY, EXPLOSION_RADIUS, false);
                //EMP ARC THE TARGET
                for (ShipAPI target : CombatUtils.getShipsWithinRange(ship.getLocation(), EXPLOSION_RADIUS)){
                    ...
                    float mult = MathUtilLS.inverse(MathUtilLS.normalize(dist,0f, EXPLOSION_RADIUS));
                ...
                Global.getCombatEngine().addSwirlyNebulaParticle(
                        ship.getLocation(), MathUtilLS.scaleVector(ship.getVelocity(),0.5f), 400f,
                        1.20f, 0.25f,0.5f,1f,
                        MiscLS.setAlpha(EMP_CORE_COLOR, 100),false);
                ...
                for (int x = 0; x < 12; x++) {
                    ...
                    particlePosTo = MathUtils.getPointOnCircumference(ship.getLocation(), radius, angle+ MathUtilLS.getRandomNumberInRangeExcludingRange(-80f, 80f,-20f,20f));
