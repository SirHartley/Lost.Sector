...
import lostsector.hullmods.Causality;
...
public class CausalityStats extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        Causality.ShipSpecificData data = (Causality.ShipSpecificData) Global.getCombatEngine().getCustomData().get("CAUSALITY_DATA_KEY" + ship.getId());
        ...
        if (state == State.ACTIVE || state == State.OUT) {
            ...
            if (!activated) {
                ...
                targets.addAll(CombatUtilsLS.getAsteroidsWithinRange(ship.getLocation(), range));
                ...
                for (CombatEntityAPI target : targets) {
                    ...
                    if (target instanceof DamagingProjectileAPI) {
                        ...
                        possibleTargets.addAll(CombatUtilsLS.getShipsWithinRange(proj.getLocation(), 2500f));
                        float closest = 9999f;
                        //distance
            ...
            if (data.energy>maxEnergy) {
                ...
                if (!sideEffect && rand<0.20f){
                    ...
                    Vector2f point1 = CombatUtilsLS.getRandomPointOnShip(ship);
