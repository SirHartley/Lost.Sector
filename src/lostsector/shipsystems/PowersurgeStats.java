...
import lostsector.hullmods.Ultracaliber;
...
public class PowersurgeStats extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        Ultracaliber.ShipSpecificData data = (Ultracaliber.ShipSpecificData) Global.getCombatEngine().getCustomData().get("CALIBER_DATA_KEY" + ship.getId());
        ...
        if (state == State.OUT) {
            ...
            if (!activated) {
                ...
                ship.getVelocity().set(MathUtilLS.scaleVector(vel, 0.50f));

                Global.getSoundPlayer().playSound(SOUND_ID,1f,0.5f, ship.getLocation(), ZERO);

                data.timer = Ultracaliber.SYS_TIME;
                activated = true;
            }
        }
