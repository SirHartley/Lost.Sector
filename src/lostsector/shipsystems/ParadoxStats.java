...
public class ParadoxStats extends BaseShipSystemScript {
	...
	static void log(final String message) {
		Global.getLogger(ParadoxStats.class).info(message);
	}
		...
		if (unapply){
			...
			if (sparkleInterval.intervalElapsed()) {
				for (int x = 0; x < 3; x++) {
					...
					float bias = MathUtilLS.BiasFunction(Math.random(), 0.2f);
					distance *= bias;
					distance += 75f;
