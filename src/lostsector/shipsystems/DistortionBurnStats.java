...
public class DistortionBurnStats extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        for (int x = 0; x < Math.round(particlesPerFrame * (Global.getCombatEngine().getElapsedInLastFrame()*60f));) {
            ...
            MathUtilLS.scaleVector(particleVel, spdMult);
        ...
        if (dmgInterval.intervalElapsed()) {
            ...
            for (CombatEntityAPI e : entities) {
                ...
                float chance = MathUtilLS.normalize(MathUtils.getDistance(ship.getLocation(), e.getLocation()), 0f, ARC_MAX_RANGE);
                chance = MathUtilLS.lerp(1f,0f, chance);
                ...
                Vector2f point = CombatUtilsLS.getNearestPointOnBounds(ship.getLocation(), e);
                ...
                if (e instanceof ShipAPI) {
                    ...
                    if ((e.getShield() != null && e.getShield().isOn() && e.getShield().isWithinArc(
                        ...
                        Global.getCombatEngine().spawnEmpArcVisual(arcPos, new SimpleEntity(arcPos), CombatUtilsLS.getNearestPointOnCollisionRadius(ship.getLocation(),e), new SimpleEntity(point), 10f, empColorFringe, empColorCore);
