...
public class Harmonics {
    ...
    public static class harmonicsSourceListener implements AdvanceableListener {
        ...
        public void advance(float amount) {
            ...
            if (timer > HarmonicsStats.MAX_DURATION){
                sourceStats.getMaxSpeed().unmodify(id);
                sourceStats.getAcceleration().unmodify(id);
                sourceStats.getDeceleration().unmodify(id);
                sourceStats.getMaxTurnRate().unmodify(id);
            ...
            float mod = MathUtilLS.inverse(MathUtilLS.normalize(timer, 0f, HarmonicsStats.MAX_DURATION));

            //engine.addFloatingText(source.getLocation(), ""+mod, 24f, Color.RED, null, 1f,1f);

            sourceStats.getMaxSpeed().modifyFlat(id, MathUtilLS.lerp(0f, maxSpeedTop, mod));
            sourceStats.getAcceleration().modifyFlat(id, MathUtilLS.lerp(0f, accelerationTop, mod));
            sourceStats.getDeceleration().modifyFlat(id, MathUtilLS.lerp(0f, decelerationTop, mod));
            sourceStats.getMaxTurnRate().modifyFlat(id, MathUtilLS.lerp(0f, maxTurnRateTop, mod));
            sourceStats.getTurnAcceleration().modifyFlat(id, MathUtilLS.lerp(0f, turnAccelerationTop, mod));
    ...
    public static class harmonicsTargetListener implements AdvanceableListener {
        ...
        public void advance(float amount) {
            ...
            stats.getTimeMult().modifyPercent(id, mod * HarmonicsStats.MAX_TIMEFLOW);

            if (target == Global.getCombatEngine().getPlayerShip()) {
                Global.getCombatEngine().maintainStatusForPlayerShip(id+"_tooltip",
                        source.getSystem().getSpecAPI().getIconSpriteName(),
                        source.getSystem().getDisplayName(),
                        "In Harmonics, Timeflow set to " + MathUtilLS.roundToTwoDecimals(stats.getTimeMult().getModifiedValue()), false);
            }
