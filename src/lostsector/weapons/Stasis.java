...
import lostsector.shipsystems.StasisStats;
...
public class Stasis {
    ...
    public static class stasisEffectListener implements AdvanceableListener {
        ...
        public void advance(float amount) {
            ...
            elapsed += amount;
            if (elapsed>StasisStats.MAX_DURATION){
            ...
            speedMod = MathUtilLS.lerp(1f, MAX_SPEED_MULT, mod);
            agilityMod = MathUtilLS.lerp(1f, MANEUVER_MULT, mod);
            weaponTurnMod = MathUtilLS.lerp(1f, WEAPON_SPEED_MULT, mod);
            ...
            if (Math.random()<1.00f * (engine.getElapsedInLastFrame()*60f)) {
                ...
                Global.getCombatEngine().addSmokeParticle(particlePos, MathUtilLS.scaleVector(particleVel, 0.1f), 5f, 0.67f, 1f,
                        COLOR);
            }
        }
    }
    ...
    public static void explode(Vector2f point, ShipAPI source) {
        //actual effect
        for (ShipAPI ship : CombatUtils.getShipsWithinRange(point, StasisStats.MAX_ON_HIT_RANGE)){
        ...
        //particle fx
        Vector2f particlePos, particleVel;
        Color color = MiscLS.randomiseColor(PARTICLE_COLOR, 25, 0,25,25,false);
        for (int x = 0; x < 150; x++) {
            ...
            Global.getCombatEngine().addSmokeParticle(particlePos, MathUtilLS.scaleVector(particleVel, 0.25f), 4f, 0.50f, 1.5f,
                    color);
        }
    }
}