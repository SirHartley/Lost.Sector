...
public class BlacksiteInfo {
    ...
    public BlacksiteInfo(SectorEntityToken entity, Random random) {
        ...
        this.count = MathHelper.getSeededRandomNumberInRange(1, 3, random);
        float mult = 1f;
        switch (this.count){
            case 1:
                mult = 0.67f;
        ...
        switch (faction){
            case Factions.PIRATES:
                this.points = MathHelper.getSeededRandomNumberInRange(150f, 175f, random) * mult;
                break;
            case Factions.LUDDIC_PATH:
                this.points = MathHelper.getSeededRandomNumberInRange(125f, 150f, random) * mult;
                break;
            case Factions.TRITACHYON:
                this.points = MathHelper.getSeededRandomNumberInRange(100f, 125f, random) * mult;
                break;
            case Ids.KESTEVEN_FACTION_ID:
                this.points = MathHelper.getSeededRandomNumberInRange(100f, 125f, random) * mult;
                break;
            case Ids.ENIGMA_FACTION_ID:
                this.points = MathHelper.getSeededRandomNumberInRange(75f, 100f, random) * mult;
                break;
