...
public class BlacksiteManager extends BaseCampaignEventListener implements EveryFrameScript {
    ...
    public void advance(float amount) {
        ...
        if (counter.val>1f) {
            ...
            for (BlacksiteInfo site : sites) {
                ...
                int neutralized = 0;
                List<FleetInfo> fleets = FleetHelper.getFleets(FLEET_ARRAY_KEY+site.id);
                ...
                FleetHelper.cleanUp(removedFleets, fleets);
                removedFleets.clear();
                //save to mem
                FleetHelper.setFleets(fleets, FLEET_ARRAY_KEY+site.id);
    ...
    private static void spawnBlacksiteFleets(BlacksiteInfo site) {
        ...
        for (int x = 0; x<count;x++) {
            ...
            List<FleetInfo> fleets = FleetHelper.getFleets(FLEET_ARRAY_KEY+target.getId());
            fleets.add(new FleetInfo(fleet, target, from));
            FleetHelper.setFleets(fleets, FLEET_ARRAY_KEY+target.getId());
