...
public class RorqualSpawner extends BaseCampaignEventListener implements EveryFrameScript {
    //
    ...
    static void log(final String message) {
        Global.getLogger(RorqualSpawner.class).info(message);
    }

    public RorqualSpawner() {
        super(false);
        ...
        if (counter.val>10f) {
            for (FleetInfo f : fleets) {
                ...
                if (visibleToPlayer && firstTime.val) {
                    //Adds our intel
                    RorqualIntel intel = new RorqualIntel(fleet);
    ...
    private static CampaignFleetAPI reinforceFleet(CampaignFleetAPI fleet, float targetStrength){
        while (fleet.getFleetPoints()<(int)targetStrength){
            ...
            if (!member.isFighterWing()) {
                addSmods(member, MathHelper.getSeededRandomNumberInRange(1,3, getRandom()), fleet.getCommanderStats());
