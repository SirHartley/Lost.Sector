	...
	public void reportEncounterLootGenerated(FleetEncounterContextPlugin plugin, CargoAPI loot) {
		...
		if (loser.getMemoryWithoutUpdate().contains(MothershipSpawner.LOOT_KEY)){
			if (loser.getFlagship()==null) {
				//completed
				QuestHelper.setCompleted(true, DEFEATED_HELIOS_KEY);
		...
		if (loser.getMemoryWithoutUpdate().contains(RorqualSpawner.LOOT_KEY)){
			if (loser.getFlagship()==null) {
				...
				float paid = RorqualSpawner.BOUNTY_PAYOUT * plugin.computePlayerContribFraction();
				setAmountPaid(paid, RorqualSpawner.DEFEAT_ID_PAID);
				Global.getSector().getPlayerFleet().getCargo().getCredits().add(paid);

				setPlayerDefeated(true, RorqualSpawner.DEFEAT_ID);
				//rep loss
				...
				loser.getMemoryWithoutUpdate().unset(RorqualSpawner.LOOT_KEY);
				loser.getMemoryWithoutUpdate().unset(MemFlags.MEMORY_KEY_MISSION_IMPORTANT);
