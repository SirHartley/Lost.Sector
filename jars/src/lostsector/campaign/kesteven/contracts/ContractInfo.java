...
public class ContractInfo {
    ...
    public void create(){
        float mult =  (3f* MathHelper.BiasFunction(random.nextDouble(), 0.50f)) * PowerLevel.get(0.20f, 0f, 2f);
        float rewardMult = MathHelper.getSeededRandomNumberInRange(0.75f, 1.5f, random);
    ...
    private void calc(int standardBase, float minRandom, float maxRandom, float mult, int standardBaseReward, float rewardMult) {
        float tempCount = MathHelper.getSeededRandomNumberInRange(standardBase * minRandom, standardBase * maxRandom, random);
        tempCount *= 1f + mult;
