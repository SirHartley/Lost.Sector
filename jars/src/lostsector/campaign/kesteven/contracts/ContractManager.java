...
public class ContractManager extends BaseCampaignEventListener implements EveryFrameScript {
    ...
    public ContractManager() {
        ...
        //init randoms
        getRandom(ContractsMission.PERSISTENT_RANDOM_KEY_ELIMINATE);
        getRandom(ContractsMission.PERSISTENT_RANDOM_KEY_RECOVERY);
    }
    ...
    public void advance(float amount) {
        ...
        if (resetCounter.val>=600f){
            //reset
            if (ContractsMission.getContract(ContractsMission.CONTRACT_KEY_ELIMINATE)!=null){
                ContractsMission.setContract(ContractsMission.CONTRACT_KEY_ELIMINATE, null);
            }
            if (ContractsMission.getContract(ContractsMission.CONTRACT_KEY_RECOVERY)!=null){
                ContractsMission.setContract(ContractsMission.CONTRACT_KEY_RECOVERY, null);
            }
    ...
    public static boolean maxContracts(List<ContractInfo> contracts, ContractInfo.ContractType type){
        ...
        int storyBonus = 1;
        if (QuestHelper.getCompleted(KestevenFlag.KESTEVEN_ENDING_DONE)){
            storyBonus = 2;
        }
