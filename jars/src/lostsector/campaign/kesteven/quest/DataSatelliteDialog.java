...
public class DataSatelliteDialog implements InteractionDialogPlugin {
	//
	//
	//
	...
	public void optionSelected(String optionText, Object optionData) {
		...
		if (optionData==OptionId.B4){
			...
			String constellation = QuestHelper.getJob5FrostTip().getConstellation().getName();
			text.addPara("Enigma, Glacier, Frozen, "+ Frost.getName()+", Heart, "+constellation,g,h,"","");
	...
	public void makeHostile(LocationAPI loc, String targetKey){
		...
		for (SectorEntityToken e : loc.getAllEntities()){
			...
			if (e.getMemoryWithoutUpdate().contains(targetKey)){
				...
				if (f.getMemoryWithoutUpdate().contains(DormantSpawner.DORMANT_KEY)){
					//un-dormant
					...
					f.getMemory().clear();
					f.getMemoryWithoutUpdate().set(DormantSpawner.DORMANT_KEY, true);
