...
public class BlackOpsManager extends BaseCampaignEventListener implements EveryFrameScript {
    ...
    public void advance(float amount) {
        ...
        if (counter.val>10f) {
            ...
            if (home!=null && FleetHelper.getFleets(FLEET_ARRAY_KEY).size() < MAX_FLEETS) {
                //rng check
    ...
    void spawnOpsFleet(SectorEntityToken loc, SectorEntityToken target, String faction) {
        ...
        simpleFleet.sMods = MathHelper.getSeededRandomNumberInRange(2, 4, random);
    ...
    public static SectorEntityToken createOpsTarget(SectorEntityToken home, FactionAPI faction){
        ...
        if (random.nextFloat()<DEFEND_CHANCE){
            ...
            //pick
            target = defend.get(MathHelper.getSeededRandomNumberInRange(0, defend.size() - 1, random));
        }
        //attack
        else {
            //has hostiles check
            if (hasHostileFactions(faction)){
                while (target==null){
                    ...
                    //pick
                    if (!attack.isEmpty()) target=attack.get(MathHelper.getSeededRandomNumberInRange(0, attack.size() - 1, random));
                }
            } else {
                //no hostiles
                //pick
