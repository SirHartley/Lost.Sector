...
public class StalkerSpawner extends BaseCampaignEventListener implements EveryFrameScript {
    ...
    public void advance(float amount) {
        ...
        if (QuestHelper.getCompleted(GlacierCommsDialog.RECOVERED_KEY) && !questSpawn.val){
    ...
    public static float getEnigmaThreatScaling(boolean stalker) {
        ...
        protDP = MathHelper.lerp(0f,0.90f, protDP);
        //op
        protOP = Math.min(protOP, 250f);
        protOP = MathHelper.normalize(protOP, 0f, 250f);
    ...
    void spawnStalkerFleets(final Integer maxAmount, float power, Random random) {
        ...
        int amount = MathHelper.getSeededRandomNumberInRange(baseMinFleets, maxAmount, random);
        for (int x = 0; x < amount; x++) {
            float combatPoints = MathHelper.getSeededRandomNumberInRange(2, 14, random);
            log("StalkerSpawner BASE " + combatPoints);
            //exponential scaling of random enigma fleet size
            float bias = 0.25f+(MathHelper.BiasFunction(random.nextDouble(), 0.33f));
            combatPoints *= bias;
            ...
            if (random.nextFloat()<0.33f) simpleFleet.sMods = MathHelper.getSeededRandomNumberInRange(1,2, random);
            if (random.nextFloat()<0.15f) simpleFleet.qualityOverride = MathHelper.getSeededRandomNumberInRange(0.60f, 0.90f, random);
            if (random.nextFloat()<0.50f) simpleFleet.noTransponder = true;
            simpleFleet.name = edgyNames.get(MathHelper.getSeededRandomNumberInRange(0, edgyNames.size() - 1, random));
