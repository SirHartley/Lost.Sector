    ...
    public static void update() {
        ...
        while (count >= REQUIRED_PER_UNLOCK){
            ...
            if (count >= REQUIRED_PER_UNLOCK) {
                String key = DEFAULT_KEYS.get(MathHelper.getSeededRandomNumberInRange(0, DEFAULT_KEYS.size() - 1, random));
                //pick until new one
                while (unlocks.contains(key)) {
                    key = DEFAULT_KEYS.get(MathHelper.getSeededRandomNumberInRange(0, DEFAULT_KEYS.size() - 1, random));
                }
                //add
                unlocks.add(key);
