                ...
                if (FleetHelper.getFleets(FLEET_ARRAY_KEY).size() < MAX_FLEETS) {
                    ...
                    if (getRandom(PERSISTENT_RANDOM_KEY).nextFloat() < chance * threatMult) {
                        Vector2f loc = new Vector2f(MathUtils.getPointOnCircumference(pf.getLocation(),
                                (pf.getSensorStrength() * 4.00f) * MathHelper.getSeededRandomNumberInRange(0.8f, 1.2f, getRandom(PERSISTENT_RANDOM_KEY)),
        ...
        switch (task){
            case INTERCEPT:
                if (StalkerSpawner.getEnigmaThreatScaling(true)>0f) {
                    ...
                    simpleFleet.name = INTERCEPT_NAMES.get(MathHelper.getSeededRandomNumberInRange(0, INTERCEPT_NAMES.size() - 1, random));
                    ...
                    simpleFleet.name = DORMANT_NAMES.get(MathHelper.getSeededRandomNumberInRange(0, DORMANT_NAMES.size() - 1, random));
                }
                break;
            case AMBUSH:
                keys.add(MemFlags.FLEET_IGNORES_OTHER_FLEETS);
                ...
                simpleFleet.sMods = MathHelper.getSeededRandomNumberInRange(0, 2, random);
                simpleFleet.assignmentText = "laying in wait";
                simpleFleet.noTransponder = true;
                simpleFleet.assignment = FleetAssignment.HOLD;
                simpleFleet.name = AMBUSH_NAMES.get(MathHelper.getSeededRandomNumberInRange(0, AMBUSH_NAMES.size() - 1, random));
                break;
            case DORMANT:
                keys.add(MemFlags.FLEET_IGNORES_OTHER_FLEETS);
                keys.add(MemFlags.FLEET_IGNORED_BY_OTHER_FLEETS);
            ...
            case GO_TO_SYSTEM:
                ...
                simpleFleet.sMods = MathHelper.getSeededRandomNumberInRange(0, 1, random);
                ...
                simpleFleet.name = MOVE_NAMES.get(MathHelper.getSeededRandomNumberInRange(0, MOVE_NAMES.size() - 1, random));
                break;
        }
