...
public class UnknownProduction extends BaseHullMod {
	...
	public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
		...
		if (unlocks.size()>=EnigmaHullmodListener.DEFAULT_KEYS.size()){
			...
			tooltip.addPara("Regenerates "+(int)RageDummy.HULL_REPAIR_PERCENT+"%% of maximum hull and recover every armor cell to 75%% if below that value. " +
					"Dissipate 50%% of current flux, cruiser size hulls are phased for "+(int)RageDummy.MAX_DISAPPEAR_TIME+" seconds. " +
					"Lose "+(int)Math.abs(RageDummy.ENRAGE_PPT_PENALTY)+"%% of remaining peak performance time.", pad, y, "");
			tooltip.addPara("Gain major improvements to many stats", pad, y, "");

			tooltip.addPara("-Top speed is increased both by "+(int)(RageDummy.ENRAGE_SPEED_BONUS)+"%% " +
					"and by an additional "+(int)(RageDummy.ENRAGE_SPEED_BONUS_FLAT)+"su/s flat ", pad, y,  (int)(RageDummy.ENRAGE_SPEED_BONUS)+"%", (int)(RageDummy.ENRAGE_SPEED_BONUS_FLAT)+"su/s");
			tooltip.addPara("-Ballistic weapon rate of fire increased and flux use reduced by "+(int)(RageDummy.ENRAGE_BALLISTIC_ROF_BONUS)+"%"+"%", 2.0f, y, (int)(RageDummy.ENRAGE_BALLISTIC_ROF_BONUS)+"%");
			tooltip.addPara("-Energy weapon damage increased by "+(int)(RageDummy.ENRAGE_ENERGY_DMG_BONUS)+"%"+"%", 2.0f, y, (int)(RageDummy.ENRAGE_ENERGY_DMG_BONUS)+"%");
			tooltip.addPara("-Flux dissipation increased by "+(int)(RageDummy.ENRAGE_FLUX_MULTI_BONUS)+"%"+"%", 2.0f, y, (int)(RageDummy.ENRAGE_FLUX_MULTI_BONUS)+"%");
			tooltip.addPara("-Shield unfold and turn rate increased by "+(int)(RageDummy.SHIELD_SPEED_BONUS)+"%"+"%", 2.0f, y, (int)(RageDummy.SHIELD_SPEED_BONUS)+"%");
		}
