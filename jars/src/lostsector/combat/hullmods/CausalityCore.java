...
public class CausalityCore extends BaseHullMod {
	...
	public void advanceInCombat(ShipAPI ship, float amount) {
		...
		if (data.energy>0){
			...
			if (Global.getCombatEngine().getPlayerShip() == ship) {
				PostProcessShader.setNoise(false, MathHelper.lerp(0f, 0.25f, effectSqrt));
				...
				float max = 8000f;
				if (data.energy>min){
					float modSynth = MathHelper.normalize(MathUtils.clamp(data.energy, min, max), min, max);
					float mult = 1.5f;
					float modNoise = MathHelper.normalize(MathUtils.clamp(data.energy, min*mult, max*mult), min*mult, max*mult);
	...
	private class IndicatorRenderer implements CombatLayeredRenderingPlugin {
		...
		public void render(CombatEngineLayers layer, ViewportAPI viewport) {
			...
			targets.addAll(CombatHelper.getProjectilesWithinRange(ship.getLocation(), range));
			targets.addAll(CombatHelper.getMissilesWithinRange(ship.getLocation(), range));
			...
			for (Pair<CombatEntityAPI, Float> target : newTargets) {
				if (target.one instanceof DamagingProjectileAPI) {
					...
					float sizeMult;
					float alpha = 1f;
					sizeMult = Math.max(1,1f+(MathHelper.smoothStep(MathHelper.lerp(0f, 1f, target.two))));
					if (target.two>0.67f){
