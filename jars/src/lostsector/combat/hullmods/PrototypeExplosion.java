...
public class PrototypeExplosion {
    //
    ...
    static void log(final String message) {
        Global.getLogger(PrototypeExplosion.class).info(message);
    }
    ...
    public static class ProtExplosionListener implements AdvanceableListener {
        ...
        public void advance(float amount) {
            ...
            for (int x = 0; x < 200*mag; x++) {
                ...
                Global.getCombatEngine().addSmokeParticle(particlePos, MathHelper.scaleVector(particleVel, 4f), 4f, 0.67f, 2.0f,
                        color);
            }
            //nebula fx
            Global.getCombatEngine().addNebulaParticle(
                    ship.getLocation(), MathHelper.scaleVector(ship.getVelocity(),0.5f), (ship.getCollisionRadius()+100f)*mag,
                    1.20f, 0.5f,1.5f,4f,
                    NEBULA_COLOR);
            ...
            for (int x = 0; x < 24*mag; x++) {
                ...
                empPosTo = MathUtils.getPointOnCircumference(ship.getLocation(), radius,
                        angle+ MathHelper.getRandomNumberInRangeExcludingRange(-70f, 70f,-30f,30f));
