...
public class AugmentedSystems extends BaseHullMod {
	...
	static void log(final String message) {
		Global.getLogger(AugmentedSystems.class).info(message);
	}
	...
	public void applyEffectsAfterShipCreation(ShipAPI ship, String id) {
		...
		if (stats.getVariant().hasHullMod(FLUX_COIL_ADJUNCT_ID)) {
			ship.addListener(new AugmentedSystemsListener.FluxCoilAdjunctListener(ship));
		}
		// BLAST DOORS
		if (stats.getVariant().hasHullMod(BLAST_DOORS_ID)) {
			ship.addListener(new AugmentedSystemsListener.BlastDoorsListener(ship));
		}
		// SAFETY OVERRIDE
		if (stats.getVariant().hasHullMod(SAFETY_OVERRIDE_ID)) {
			ship.addListener(new AugmentedSystemsListener.SafetyOverrideListener(ship));
		}
		// UNSTABLE INJECTOR
		if (stats.getVariant().hasHullMod(UNSTABLE_INJECTOR_ID)) {
			ship.addListener(new AugmentedSystemsListener.UnstableInjectorListener(ship));
		}
		// NAV RELAY
		if (stats.getVariant().hasHullMod(NAV_RELAY_ID)) {
			ship.addListener(new AugmentedSystemsListener.NavRelayListener(ship));
		}
		//OPERATIONS CENTER
		if (stats.getVariant().hasHullMod(OPERATIONS_CENTER_ID)) {
			ship.addListener(new AugmentedSystemsListener.OpCenterListener(ship));
		}
	}
	...
	public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
		...
		boolean showAll = ship == null || Global.getSettings().isShowingCodex();

		// HULLMODS
		tooltip.addSectionHeading("Current Augments", Alignment.MID, pad);
		// FLUX COIL ADJUNCT
		if (showAll || ship.getVariant().hasHullMod(FLUX_COIL_ADJUNCT_ID)) {
		...
		// TURRET GYROS
		if (showAll || ship.getVariant().hasHullMod(TURRET_GYROS_ID)) {
		...
		// ECM
		if (showAll || ship.getVariant().hasHullMod(ECM_ID)) {
		...
		// HARDENED SHIELDS
		if (showAll || ship.getVariant().hasHullMod(HARDENED_SHIELDS_ID)) {
		...
		// HEAVY ARMOR
		if (showAll || ship.getVariant().hasHullMod(HEAVY_ARMOR_ID)) {
		...
		// UNSTABLE INJECTOR
		if (showAll || ship.getVariant().hasHullMod(UNSTABLE_INJECTOR_ID)) {
		...
		// SOLAR SHIELDING
		if (showAll || ship.getVariant().hasHullMod(SOLAR_SHIELDING_ID)) {
			TooltipMakerAPI text = tooltip.beginImageWithText("graphics/icons/hullsys/damper_field.png", 32.0f);
		...
		// REPAIR UNIT
		if (showAll || ship.getVariant().hasHullMod(REPAIR_UNIT_ID)) {
		...
		// IPDAI
		if (showAll || ship.getVariant().hasHullMod(IPDAI_ID)) {
		...
		// HIGH RESOLUTION SENSORS
		if (showAll || ship.getVariant().hasHullMod(HIGH_RESOLUTION_SENSORS_ID)) {
			TooltipMakerAPI text = tooltip.beginImageWithText("graphics/icons/hullsys/interdictor_array.png", 32.0f);
		...
		// SHIELD SHUNT
		if (showAll || ship.getVariant().hasHullMod(SHIELD_SHUNT_ID)) {
		...
		// ACCELERATED SHIELDS
		if (showAll || ship.getVariant().hasHullMod(ACCELERATED_SHIELDS_ID)) {
			TooltipMakerAPI text = tooltip.beginImageWithText("graphics/icons/hullsys/fortress_shield.png", 32.0f);
		...
		// STABILIZED SHIELDS
		if (showAll || ship.getVariant().hasHullMod(STABILIZED_SHIELDS_ID)) {
			TooltipMakerAPI text = tooltip.beginImageWithText("graphics/icons/hullsys/fortress_shield.png", 32.0f);
		...
		// EXTENDED SHIELDS
		if (showAll || ship.getVariant().hasHullMod(EXTENDED_SHIELDS_ID)) {
			TooltipMakerAPI text = tooltip.beginImageWithText("graphics/icons/hullsys/fortress_shield.png", 32.0f);
		...
		// FRONT CONVERSION
		if (showAll || ship.getVariant().hasHullMod(FRONT_CONVERSION_ID)) {
			TooltipMakerAPI text = tooltip.beginImageWithText("graphics/icons/hullsys/fortress_shield.png", 32.0f);
		...
		// OMNI CONVERSION
		if (showAll || ship.getVariant().hasHullMod(OMNI_CONVERSION_ID)) {
			TooltipMakerAPI text = tooltip.beginImageWithText("graphics/icons/hullsys/fortress_shield.png", 32.0f);
		...
		// BLAST DOORS
		if (showAll || ship.getVariant().hasHullMod(BLAST_DOORS_ID)) {
			TooltipMakerAPI text = tooltip.beginImageWithText("graphics/icons/hullsys/damper_field.png", 32.0f);
		...
		// OPERATIONS CENTER
		if (showAll || ship.getVariant().hasHullMod(OPERATIONS_CENTER_ID)) {
		...
		// RECOVERY SHUTTLES
		if (showAll || ship.getVariant().hasHullMod(RECOVERY_SHUTTLES_ID)) {
		...
		// SAFETY OVERRIDE
		if (showAll || ship.getVariant().hasHullMod(SAFETY_OVERRIDE_ID)) {
		...
		// NAV RELAY
		if (showAll || ship.getVariant().hasHullMod(NAV_RELAY_ID)) {
			TooltipMakerAPI text = tooltip.beginImageWithText("graphics/icons/hullsys/reserve_deployment.png", 32.0f);
		...
		// IEA
		if (showAll || ship.getVariant().hasHullMod(IEA_ID)) {
			TooltipMakerAPI text = tooltip.beginImageWithText("graphics/icons/hullsys/burn_drive.png", 32.0f);
		...
		// FLUX DISTRIBUTOR
		if (showAll || ship.getVariant().hasHullMod(FLUX_DISTRIBUTOR_ID)) {
		...
		// INERTIAL
		if (showAll || ship.getVariant().hasHullMod(INERTIAL_ID)) {
			TooltipMakerAPI text = tooltip.beginImageWithText("graphics/icons/hullsys/temporal_shell.png", 32.0f);
		...
		// VOLATILE
		if (showAll || ship.getVariant().hasHullMod(VOLATILE_ID)) {
		...
		// AWM
		if (showAll || ship.getVariant().hasHullMod(ARMORED_WEAPON_MOUNT_ID)) {
			TooltipMakerAPI text = tooltip.beginImageWithText("graphics/icons/hullsys/damper_field.png", 32.0f);
		...
		// CONVERTED HANGAR
		if (showAll || ship.getVariant().hasHullMod(CONVERTED_HANGAR_ID)) {
			TooltipMakerAPI text = tooltip.beginImageWithText("graphics/icons/hullsys/damper_field.png", 32.0f);
			text.addPara("CONVERTED HANGAR", 0.0f, tc, "CONVERTED HANGAR");
			if (!showAll && ship.getHullSize()!= HullSize.DESTROYER) {
		...
		// BIG BATS
		if (showAll || ship.getVariant().hasHullMod(Ids.HIGH_CAPACITANCE_BANKS_HULLMOD_ID)) {
			TooltipMakerAPI text = tooltip.beginImageWithText("graphics/icons/hullsys/high_energy_focus.png", 32.0f);
			text.addPara("HIGH CAPACITANCE BANKS", 0.0f, tc, "HIGH CAPACITANCE BANKS");
			text.addPara("Increases system charge regen rate by "+(int)HighCapacitanceBanks.AUGMENT_RECHARGE_BONUS+"%"+"%.", 0.0f, y, (int)HighCapacitanceBanks.AUGMENT_RECHARGE_BONUS+"%");
			tooltip.addImageWithText(pad);
		}
		// CRITICAL POINT PROTECTION
		if (showAll || ship.getVariant().hasHullMod(Ids.CRITICAL_POINT_PROTECTION_HULLMOD_ID)) {
			TooltipMakerAPI text = tooltip.beginImageWithText("graphics/icons/hullsys/damper_field.png", 32.0f);
