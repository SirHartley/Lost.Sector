...
import lostsector.combat.hullmods.PullbackDummy;
...
public class PullbackAI implements ShipSystemAIScript {
    ...
    public void advance(float amount, Vector2f missileDangerDir, Vector2f collisionDangerDir, ShipAPI target) {
        ...
        PullbackDummy.ShipSpecificData data = (PullbackDummy.ShipSpecificData) Global.getCombatEngine().getCustomData().get("PULLBACK_DATA_KEY" + ship.getId());
        if(data==null)return;
        ...
        if (tracker.intervalElapsed()) {
            for (Vector2f point : points) {
                ...
                possibleTargets.addAll(CombatHelper.getProjectilesWithinRange(ship.getLocation(), 500f));
                possibleTargets.addAll(CombatHelper.getMissilesWithinRange(ship.getLocation(), 400f));
