...
import lostsector.combat.systems.OmniLoadStats;
...
public class OmniLoadAI implements ShipSystemAIScript {
    ...
    public void init(ShipAPI ship, ShipSystemAPI system, ShipwideAIFlags flags, CombatEngineAPI engine) {
        ...
        for (WeaponAPI w : ship.getAllWeapons()) {
            if (!OmniLoadStats.validWeapon(w)) continue;
    ...
    public void advance(float amount, Vector2f missileDangerDir, Vector2f collisionDangerDir, ShipAPI target) {
        ...
        if (timer.intervalElapsed()) {
            ...
            float fluxMod = MathHelper.normalize(Math.min(maxFlux, 1500f), 0f, 1500f);
            //fluxed
            decisionLevel -= MathHelper.lerp(0f,40f*fluxMod, ship.getFluxLevel());
            //high ammo
            ...
            //cooldown
            float norm = MathHelper.normalize(currCooldown, 0f, maxCooldown);
            decisionLevel += MathHelper.lerp(0f, cooldownBonus, norm);
            //ammo
            if (ammoBonus>0f) {
                norm = MathHelper.normalize(currAmmo, 0, maxAmmo);
                decisionLevel += MathHelper.lerp(ammoBonus, 0f, norm);
            }
            ...
            for (ShipAPI t : CombatUtils.getShipsWithinRange(ship.getLocation(), 500f)){
                ...
                decisionLevel += MathHelper.inverse(t.getHullLevel()) * 30f;
