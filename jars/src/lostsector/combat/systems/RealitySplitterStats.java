...
public class RealitySplitterStats extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        shipsInBounds.addAll(CombatHelper.getShipsWithinRange(ship.getLocation(), ship.getCollisionRadius()));
        ...
        if (state == State.ACTIVE && OOB || timer<2.5f){
            ...
            if (Math.random() < 1.0f * (engine.getElapsedInLastFrame()*60f)) {
                ...
                particleVel = MathHelper.scaleVector(particleVel, 0.5f);
        ...
        if (state == State.OUT) {
            ...
            ship.setExtraAlphaMult(1f);
                ...
                if (Explosions) {
                    ...
                    //AOE DAMAGE
                    CombatHelper.applyAOEDamage(ship, null, ship.getLocation(), EXPLOSION_DAMAGE_AMOUNT, DamageType.HIGH_EXPLOSIVE, EXPLOSION_PUSH_RADIUS, false);
                    ...
                    for (CombatEntityAPI tmp : CombatUtils.getEntitiesWithinRange(ship.getLocation(),EXPLOSION_PUSH_RADIUS)){
                        if (tmp == ship || tmp.getCollisionClass()==CollisionClass.NONE) continue;
                        ...
                        if (tmp instanceof ShipAPI) {
                            ...
                            if ((victim.getShield() != null && victim.getShield().isOn() && victim.getShield().isWithinArc(
                                ...
                                for (int x = 0; x < 5; x++) {
                                    EmpArcEntityAPI arc = engine.spawnEmpArcPierceShields(ship, CombatHelper.getRandomPointOnShip(victim),
                                            empTarget,
