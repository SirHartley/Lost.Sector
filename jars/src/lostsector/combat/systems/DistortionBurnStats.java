...
public class DistortionBurnStats extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        for (int x = 0; x < Math.round(particlesPerFrame * (Global.getCombatEngine().getElapsedInLastFrame()*60f));) {
            ...
            MathHelper.scaleVector(particleVel, spdMult);
        ...
        if (dmgInterval.intervalElapsed()) {
            ...
            for (CombatEntityAPI e : entities) {
                ...
                float chance = MathHelper.normalize(MathUtils.getDistance(ship.getLocation(), e.getLocation()), 0f, ARC_MAX_RANGE);
                chance = MathHelper.lerp(1f,0f, chance);
                ...
                Vector2f point = CombatHelper.getNearestPointOnBounds(ship.getLocation(), e);
                ...
                if (e instanceof ShipAPI) {
                    ...
                    if ((e.getShield() != null && e.getShield().isOn() && e.getShield().isWithinArc(
                        ...
                        Global.getCombatEngine().spawnEmpArcVisual(arcPos, new SimpleEntity(arcPos), CombatHelper.getNearestPointOnCollisionRadius(ship.getLocation(),e), new SimpleEntity(point), 10f, empColorFringe, empColorCore);
