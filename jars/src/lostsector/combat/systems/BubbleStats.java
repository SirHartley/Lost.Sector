...
import lostsector.combat.hullmods.AbsorbSystem;
...
public class BubbleStats extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        AbsorbSystem.ShipSpecificData data = (AbsorbSystem.ShipSpecificData) Global.getCombatEngine().getCustomData().get("ABSORPTION_DATA_KEY" + ship.getId());
        if (!updated) {
            activated = false;
        ...
        if (state == State.OUT || state == State.ACTIVE) {
            if (!activated) {
                ...
                data.timer = AbsorbSystem.SYS_TIME;
                activated = true;
