...
public class EMBlastStats extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        if (state == State.IN) {
            // SPEED
            ship.getMutableStats().getMaxSpeed().modifyMult(id, MathHelper.inverse(effectLevel/2.0f));
            // tank
            ...
            //make the AI scared
            for (ShipAPI potentialTarget : CombatHelper.getShipsWithinRange(ship.getLocation(), 450f)){
        ...
        if (state == State.ACTIVE) {
            ...
            if (!explosions) {
                //AOE DAMAGE
                CombatHelper.applyAOEDamage(ship, null, ship.getLocation(), EXPLOSION_DAMAGE_AMOUNT, DamageType.ENERGY, EXPLOSION_RADIUS, false);
                //EMP ARC THE TARGET
                for (ShipAPI target : CombatUtils.getShipsWithinRange(ship.getLocation(), EXPLOSION_RADIUS)){
                    ...
                    float mult = MathHelper.inverse(MathHelper.normalize(dist,0f, EXPLOSION_RADIUS));
                ...
                for (int x = 0; x < 12; x++) {
                    ...
                    particlePosTo = MathUtils.getPointOnCircumference(ship.getLocation(), radius, angle+ MathHelper.getRandomNumberInRangeExcludingRange(-80f, 80f,-20f,20f));
