...
import lostsector.combat.hullmods.UltracaliberBattery;
import lostsector.helper.MathHelper;
import org.lwjgl.util.vector.Vector2f;

public class PowerSurgeStats extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        UltracaliberBattery.ShipSpecificData data = (UltracaliberBattery.ShipSpecificData) Global.getCombatEngine().getCustomData().get("CALIBER_DATA_KEY" + ship.getId());
        ...
        if (state == State.OUT) {
            ...
            if (!activated) {
                ...
                ship.getVelocity().set(MathHelper.scaleVector(vel, 0.50f));

                Global.getSoundPlayer().playSound(SOUND_ID,1f,0.5f, ship.getLocation(), ZERO);

                data.timer = UltracaliberBattery.SYS_TIME;
                activated = true;
            }
        }
