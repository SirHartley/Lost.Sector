...
public class ArcGeneratorEffect implements EveryFrameWeaponEffectPlugin {

    public static float TARGET_FIND_STEP_LENGTH = 0.05f;
    public static float LIGHTNING_JUMP_RANGE_PERCENTAGE = 0.35f;
    public static Color LIGHTNING_CORE_COLOR = new Color(212, 217, 255);
    public static Color LIGHTNING_FRINGE_COLOR = new Color(114, 0, 195);
    public static final Color ARC_FRINGE_COLOR = new Color(250, 80, 100);
    public static final Color ARC_CORE_COLOR = new Color(97, 80, 250);
    ...
    private final IntervalUtil effectInterval = new IntervalUtil(0.1f, 0.2f);
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        while (firstTarget == null && iter < 1f) {
            ...
            for (CombatEntityAPI potentialTarget : targetList) {
                ...
                if (!(potentialTarget instanceof ShipAPI) && !(potentialTarget instanceof MissileAPI)) continue;
                if (potentialTarget.getOwner() == weapon.getShip().getOwner()) continue;
                if (MathUtils.getDistance(potentialTarget.getLocation(), weaponFirePoint) - (potentialTarget.getCollisionRadius()*0.9f) > range) continue;
        ...
        if (firstTarget == null) {
            ...
            Global.getCombatEngine().spawnEmpArc(weapon.getShip(), weaponFirePoint, weapon.getShip(), new SimpleEntity(targetPoint),
                    ...
                    MathUtils.getRandomNumberInRange(6f, 12f), // thickness of the lightning bolt
        ...
        while (damageThisShot > 1f) {
            ...
            for (CombatEntityAPI potentialTarget : targetList) {
                //Checks for dissallowed targets, and ignores them
                if (!(potentialTarget instanceof ShipAPI) && !(potentialTarget instanceof MissileAPI) || potentialTarget.getOwner() == weapon.getShip().getOwner()) {
                    continue;
                }
                ...
                //modules
                if (potentialTarget instanceof ShipAPI){
                    if (((ShipAPI) potentialTarget).isStation())continue;
                    if (((ShipAPI) potentialTarget).isStationModule())continue;
                }
            ...
            Global.getCombatEngine().spawnEmpArc(weapon.getShip(), firingPoint, tempPreviousTarget, currentTarget,
                    ...
                    100000f, //Max range
                    "tachyon_lance_emp_impact", //Impact sound
                    4f + 6f * (damageThisShot / weapon.getDamage().getDamage()), // thickness of the lightning bolt
                    LIGHTNING_CORE_COLOR, //Central color
                    LIGHTNING_FRINGE_COLOR //Fringe Color
            );
