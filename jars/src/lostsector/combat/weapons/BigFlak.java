...
public class BigFlak {
        ...
        public void advance(float amount) {
            ...
            for (CombatEntityAPI c : CombatUtils.getEntitiesWithinRange(projectile.getLocation(), fuse)){
                ...
                if (c instanceof ShipAPI) {
                    ...
                    if (MathUtils.getDistance(CombatHelper.getNearestPointOnBounds(projectile.getLocation(), c), projectile.getLocation()) < fuse){
                        trigger(engine, point);
                        return;
                    }
                }
    ...
    public static void explode(Vector2f point, ShipAPI source, @Nullable CombatEntityAPI mainTarget) {
        //actual effect
        CombatHelper.applyAOEDamage(source, mainTarget, point, BigFlakStats.EXPLOSION_DAMAGE, DamageType.HIGH_EXPLOSIVE, BigFlakStats.EXPLOSION_RADIUS,false);
        ...
        for (int x = 0; x < 125; x++) {
            ...
            Global.getCombatEngine().addSmokeParticle(particlePos, MathHelper.scaleVector(particleVel, 2f), 4f, 0.50f, 2.0f,
                    color);
        }
    }
}
