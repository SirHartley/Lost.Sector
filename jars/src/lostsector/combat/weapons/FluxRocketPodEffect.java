...
public class FluxRocketPodEffect implements EveryFrameWeaponEffectPlugin {
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        for (DamagingProjectileAPI proj : engine.getProjectiles()) {
            if (proj.getWeapon() != weapon || !PROJ_IDS.contains(proj.getProjectileSpecId())) {
                continue;
            }
            ...
            for (int x = 0; x < 3; x++) {
                ...
                float bias = MathHelper.BiasFunction(Math.random(),0.4f);
                distance *= bias;
                distance += 5f;
