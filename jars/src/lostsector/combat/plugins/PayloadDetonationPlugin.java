package lostsector.combat.plugins;
...
public class PayloadDetonationPlugin extends BaseEveryFrameCombatPlugin {
    ...
    static void log(final String message) {
        Global.getLogger(PayloadDetonationPlugin.class).info(message);
    }
    ...
    void kaboom(AntimatterPayload.ShipSpecificData data, ShipAPI ship) {
        CombatEngineAPI engine = Global.getCombatEngine();
        if (Global.getCombatEngine().isPaused()) {
            return;
        }
        ...
        if (data.timer < 2.5f) {
            ...
            //nebula
            if (!data.doOnce) {
                Vector2f vel = MathHelper.scaleVector(ship.getVelocity(), 0.25f);
                for (int x = 0; x < 10; x++) {
            ...
            for (int x = 0; x < 4; x++) {
                ...
                float targetLength = MathHelper.lerp(600, 0, timeOffset);
