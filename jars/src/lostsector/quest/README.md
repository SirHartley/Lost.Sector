...
Read this file in full before changing quest code, quest rules rows, quest intel, quest bar events or quest fleets, and again after a context compaction. It is also the specification for the framework classes.
...
Every component below is implemented. A change that adds a component adds its row in the same commit and corrects any signature in this file that the code had to change. A component listed as *planned* does not exist yet: do not write code against it and do not write a substitute; build it first, or stop and report.

| Component | Classes | Status |
|---|---|---|
| Core: definitions, state, store, manager, transitions, marks, claims | `Quest`, `QuestStage`, `QuestState`, `QuestModule`, `Declarations`, `QuestContext`, `QuestManager`, `QuestStore`, `QuestCatalog`, `Quests`, `QuestDialogs`, `NoFlags` | implemented |
| Events: listeners, daily tick, frame hook, fleet detection, reputation changes | `QuestManager` | implemented |
| Fleets | `QuestFleets`, `QuestFleet`, `FleetRole`, `FleetOrders` | implemented |
| Rules command, tokens, people, rewards | `nskr_quest`, `QuestVerbs`, `QuestTokens`, `QuestPeople`, `QuestRewards` | implemented |
| Presentation spec (vanilla commands per effect) | [DIALOGUE.md](../../../../docs/DIALOGUE.md#presentation-in-rules) | implemented |
| Verbs no vanilla command covers: `confirm`, `engage` | `nskr_quest` | implemented |
| Intel and rules text outside dialogs | `QuestIntel`, `QuestIntels`, `IntelSpec`, `QuestText`, `RuleScript` | implemented |
| Raid objectives | `QuestRaidObjective`, `onRaidObjectives`, `d.raid`, `ctx.raidObjective` | implemented |
| Rules check tool | `lostsector.quest.dev.RulesCheck` | implemented |
| Dev menu and stage jumps | `nskr_questDev`, `QuestDevTools`, `QuestManager.jump`, `QuestContext.jumpTarget` | implemented |
| Shared modules | `lostsector.quest.modules`: `InterceptEncounter`, `PayOffEncounter`, `BountyEncounter` | implemented |
...
1. **Use the framework for everything it covers.** The table lists the only allowed way to do each job. Writing a second way is a defect, even when it is shorter.
   ...
   | Show a bar event | An `AddBarEvents` row with `AddBarEvent` | `PortsideBarData.addEvent`, `BaseBarEvent` subclasses |
...
| `onFleetDetected` | The player's view of a fleet of this quest's roles changes level; `VisibilityLevel.NONE` when it drops out of view | Sightings: a fleet the player has found |
...
public final class QuestContext<S, T> {
    ...
    @SafeVarargs public final void mark(MarketAPI market, S... scope);
    ...
    public void continueDialog(SectorEntityToken target, String trigger);   // hands the open dialog to a rules dialog on the target
...
| `reportDetectedEntity(entity, level)` | `DetectedEntityListener` | `onFleetDetected` | Active modules of the fleet's owning quest |
...
public final class QuestFleets {
    ...
    public QuestFleet registered(CampaignFleetAPI fleet);                 // the registered fleet of any quest; null when none
    public void clearMemory(CampaignFleetAPI fleet);                      // clears fleet memory, keeps the framework's keys
...
public final class FleetOrders {
    ...
    public static FleetOrders keep();                 // SimpleFleet's assignment; HOLD ("holding") when it has none
    public static FleetOrders patrolHomeAfterChase(String patrolText);   // keep(); back to FleetInfo.home after a chase
    ...
    public static FleetOrders leave(String textPrefix);   // with "<textPrefix> <market name>" as the assignment text
    public static FleetOrders withdraw();             // keep the last assignment, despawn once out of the player's sight
    public static FleetOrders expedition(float prepareDays, float returnAfterDays);   // prepare at home, orbit FleetInfo.target, return
    public static FleetOrders huntInSystem();         // intercept the player while seen, else patrol its own system
    public static FleetOrders defendSystem(String text);   // intercept the player in the fleet's star system, else orbit its center
    ...
    public FleetOrders reinforceBelow(float fraction, BiConsumer<FleetInfo, Random> reinforce);   // patrol orders only
}
...
- **Clearing fleet memory.** Code that clears a fleet's whole memory, as waking a dormant fleet does, calls `clearMemory(fleet)` instead of `getMemory().clear()`. For a registered fleet of any quest it writes back what registration keeps there: `OWNER_KEY`, `ROLE_KEY`, `RECORD_KEY`, the role flag and, when the owning quest declares the role, its config and defeat trigger (`QuestManager.declaredRole`). Other keys, including vanilla defeat triggers, are gone as with a plain clear. `reassign` needs the owner key, so a registered fleet is cleared this way before it takes a new role; `KestevenSatelliteModule` does it for a woken fleet that was already a quest fleet.
...
- **Orders.** Every 0.1 days of campaign time (`Misc.getDays(amount)`, unpaused frames), the manager reads the list and applies each live fleet's `FleetOrders` by calling `FleetHelper.gotoAndInterceptPlayerAI`, `FleetHelper.guardTargetAI`, for `keep()` `FleetHelper.keepAssignmentAI`, for `patrolHomeAfterChase(...)` `FleetHelper.patrolHomeAfterChaseAI`, for `leave()` `FleetHelper.goToTargetAndDespawnAI`, for `raid(...)` `FleetHelper.raidTargetAI`, for `expedition(...)` `FleetHelper.expeditionAI`, for `huntInSystem()` `FleetHelper.huntInSystemAI` or, for `defendSystem(...)`, `FleetHelper.defendSystemAI`, the cadence those methods are written for. The list is read once per interval, not every frame, because `FleetHelper.getFleets` goes through the sector's `getMemory()`, which runs every campaign plugin's `updateGlobalFacts`. Orders do not depend on the quest being available. A fleet whose quest or role is unknown gets no orders and is logged once. The manager also advances `FleetInfo.age` in days. A behavior these methods do not provide is added to `FleetHelper`, then to `FleetOrders`; never to a quest.
- **Withdrawal.** `withdrawWhenBeaten()` (below a quarter of the spawn strength, `FleetHelper.isBeaten`) `withdrawAfter(days)` (`FleetInfo.age` above the days) and `withdrawWhen(condition)` (the condition, tested with the fleet's `FleetInfo` on every order interval, in the game only; `FleetHelper.isEmptied`, no fleet points left, is the condition of the Kesteven revenge fleets) replace the orders with `FleetHelper.despawnOutOfSight`: the fleet keeps its last assignment and despawns with `PLAYER_FAR_AWAY` once it is farther from the player than `getMaxSensorRangeHyper()`, measured between `getLocationInHyperspace()` positions. The despawn reaches `onFleetGone`. `withdraw()` applies the same rule at once, for a role a fleet takes when its part is over, such as a beaten bounty fleet.
- **Leaving.** `leave()` issues `GO_TO_LOCATION_AND_DESPAWN` to `FleetInfo.target` with the text "returning to <market name>" (the entity name without a market) whenever the fleet has another assignment, after the same `STANDING_DOWN` handling as the other two methods. It does nothing while the target is null, so the quest sets the target before the fleet takes the role. `leave(textPrefix)` uses `textPrefix` instead of "returning to".
- **Keep.** `keep()` runs `FleetHelper.keepAssignmentAI`: the fleet keeps the assignment `SimpleFleet` gave it, and one left without an assignment gets `HOLD` at its position ("holding"), the safety step of the other methods. `patrolHomeAfterChase(patrolText)` adds one rule (`FleetHelper.patrolHomeAfterChaseAI`): a fleet whose assignment is `INTERCEPT` while it is not in the player's location gets `MemFlags.MEMORY_KEY_SAW_PLAYER_WITH_TRANSPONDER_ON`, so it stays aggressive to the player, and patrols `FleetInfo.home` (`PATROL_SYSTEM`, `patrolText`). These are the orders of the Kesteven job 4 fleets (`KestevenJob4Module`).
...
- **Patrol.** `FleetOrders.patrolMarkets(factionId, switchDays, patrolText)` runs `FleetHelper.patrolMarketsAI`. `FleetInfo.target` starts as `FleetInfo.home` (the spawn location). In the target's star system the fleet patrols it (`PATROL_SYSTEM`, `patrolText`) and has `MemFlags.FLEET_IGNORES_OTHER_FLEETS` unset; `FleetInfo.patrolArrivedAge` records the age at which it arrived. Once it has been there longer than `switchDays`, the target becomes a random market of the faction (`SystemHelper.getRandomFactionMarket` with the orders random), after an `ORBIT_PASSIVE` order that resets the patrol. Outside the target's system it moves there with `GO_TO_LOCATION` ("moving to <market name>") and ignores other fleets on the way. Nothing changes while it is in hyperspace. With `reinforceBelow(fraction, reinforce)`, a fleet below that fraction of its spawn strength at a switch runs `reinforce` with the orders random.
- **Expedition.** `FleetOrders.expedition(prepareDays, returnAfterDays)` runs `FleetHelper.expeditionAI` by `FleetInfo.age`: below `prepareDays` the fleet orbits `FleetInfo.home` (`ORBIT_PASSIVE`, "preparing") while in home's location; above it, it goes to `FleetInfo.target` (`GO_TO_LOCATION`, "moving to location") while not in the target's location; below `returnAfterDays` in the target's location it orbits the target ("on expedition", issued again on every order interval); above it, it goes home ("returning to <home name>") and, in home's location, adds an orbit there ("standing down") and stays. The five checks run in that order against the assignment the fleet had at the start of the interval; a fleet with no assignment first gets `HOLD` ("holding"), and a fleet with `MemFlags.FLEET_BUSY` gets nothing. It never despawns the fleet; the quest gives it another role when its part is over. `spawn` sets `FleetInfo.home` to the spec's location and leaves the target null, so the quest sets `fleet.info().target` right after spawning. This is the schedule of the Kesteven job 3 expedition (`KestevenJob3Module`, 10 and 70 days).
- **Hunt.** `FleetOrders.huntInSystem()` runs `FleetHelper.huntInSystemAI`: while the player is visible to the fleet's sensors (`isVisibleToSensorsOf`, false in another location) the fleet intercepts them (`INTERCEPT`, "intercepting your fleet"); otherwise it patrols its star system's centre (`PATROL_SYSTEM`, "patrolling"), or the point where it is when it is in hyperspace. Each order is issued only when the fleet does not already have that assignment type. A fleet with no assignment first gets `HOLD` ("holding"); a `STANDING_DOWN` fleet gets the do-not-attack handling of the other methods and still takes the intercept or patrol in the same interval; a fleet with no fleet points left gets no orders and despawns once out of the player's sight. It needs no target or home, so it suits an adopted fleet. This is the behavior of the Kesteven questline's woken satellite guards (`KestevenSatelliteModule`, with `withdrawAfter(30)`).
- **Defending a system.** `FleetOrders.defendSystem(text)` runs `FleetHelper.defendSystemAI`: after the null-assignment `HOLD` and the `STANDING_DOWN` handling, which it does not stop at, the fleet gets `INTERCEPT` on the player while the player is in the fleet's star system and `ORBIT_PASSIVE` around the system's center otherwise, both with `text`; each is issued only when the fleet does not already have that assignment type. Unlike `huntInSystem()` it follows the player's location, not sensor contact. Nothing is issued while the fleet is outside a star system. This is the Cache guardian's behavior (`KestevenCacheModule`, with `withdrawWhen` for a guardian without prototypes).
...
- **Fleet conversations.** Rows on `OpenCommLink` or `BeginFleetEncounter` test the role flag, as in `$nskr_kq_ttCollector`. See [Entry points](../../../../docs/RULES_WRITING.md#entry-points).
...
| A market whose whole dialog a scene replaces while the quest needs it, such as Eliza's port or an ending | An `OpenInteractionDialog` row at `score:10000` gated by a check, which outranks every vanilla market opening ([project routing](../../../../docs/RULES.md#project-routing)) | Declared check |
...
| A Java dialog that ends in a quest scene without closing its window, such as a fleet encounter delegate's `notifyLeave` | `ctx.continueDialog(target, trigger)` on a context carrying that dialog | Declared trigger |

Claims write the trigger to entity memory under `QuestDialogs.CLAIM_KEY` (`$nskr_questDialog`). `CorePlugin` has one route for all of them: when the target's memory holds that key, it returns `new RuleBasedInteractionDialogPluginImpl(trigger)` at `PickPriority.MOD_GENERAL`. Claims are cleared with their scope like marks. Claiming an entity that already holds another claim logs an error and replaces it; releasing unsets the key only while it still holds that claim's trigger. Do not claim a market's entity: a claim replaces the market dialog in every visit of its scope. Add options to the market menu, or take the dialog over with a gated `OpenInteractionDialog` row for the visits that need it.
...
`ctx.continueDialog` sets the dialog's interaction target, replaces its plugin with the same rules plugin and calls the plugin's `init`, which fires the trigger at once, the way the `engage` verb hands a dialog to a fleet encounter. It needs a context with a dialog, from a rules call or from `QuestManager.context(questId, ruleId, dialog, memoryMap, args)`, and logs an error without one. The Cache guardian's encounter uses it to continue with the command core (`KestevenQuest.showCacheCore`).
...
3. Every exit calls `BarCMD returnFromEvent`, which returns to the bar list and clears the active person. `BarCMD returnFromEvent true` shows a Continue option first, as a finished Java bar event does.

Details and exits are in [Entry points](../../../../docs/RULES_WRITING.md#entry-points) and [Exits and returns](../../../../docs/RULES_WRITING.md#exits-and-returns). A check verb decides whether the event appears at a market (`nskr_quest kq check partyHere`); the market to use is picked when the stage starts and stored in the state. The person is created in `onStart` of the module, or by the hook that opens a recurring offer (`HintsTipModule.onDay`), not when the bar opens.
...
public final class QuestIntels {
    ...
    public void close(String key);                                            // ends after the vanilla delay; no status change, no message
    public void end(String key);                                              // ends at once
    public boolean isShown(String key);
    public QuestIntels record(String record);                                 // the same methods for one record's entries
}
...
public final class IntelSpec {                        // returned by d.intel; options are set in declare() only
    ...
    public IntelSpec deletable();                     // delete button in the description once completed, failed or closed
    public IntelSpec deletableWhen(String check);     // delete button while the declared check passes, also while active
...
- **`show`** adds a `QuestIntel` with `IntelManagerAPI.addIntel(intel, false, textPanel)`: with a dialog open, the posting message prints in its text panel; without one, the game posts a campaign message. It also adds the entry as a sector script, because the intel manager does not advance its entries: the script counts down the vanilla end delay, and the sector drops it once the entry has ended (`isDone()`). `advanceImpl` does nothing. `show` is a no-op while the entry is active. An undeclared key, or an icon that `SettingsAPI.getSpriteName("campaignMissions", icon)` does not find, logs an error and shows nothing.
- **Records.** A record quest shows one entry per record under one declared key. `ctx.intel().record(id)` returns a `QuestIntels` whose methods all address the entry of their key and that record; `ctx.intel()` addresses the entries without a record, so its `end(key)` leaves the entries of records alone. An empty record id logs an error and returns the unbound object. Rows select on `$nskr_intel_key` as usual; checks and tokens read the record with `ctx.intelRecord()`, which returns the scratch key `$nskr_intel_record` of the entry being read, and null outside intel text or for an entry without a record. Quest `hint` is the example.
- **Active entry.** An entry is active until `complete`, `fail`, `close` or `end`. `isShown` is true only for an active entry, so a completed entry still counting down its delay does not stop `show` from adding a new one.
- **`update`** sends an update message through `sendUpdateIfPlayerHasIntel(updateKey, textPanel)`: in the dialog's text panel when one is open, otherwise as a campaign message, which the game sends only for an entry the player has and that is not hidden. `complete` and `fail` set the status, send an update (with an empty update key unless one is given) and call `endAfterDelay()` (`BaseIntelPlugin.getBaseDaysAfterEnd()`, 3 days). `close` calls `endAfterDelay()` only: the status stays and no message is sent, for an entry that simply runs out, such as a location the player has reached. `end` calls `endImmediately()` on the entry of the key (and record), active or ending; the intel manager removes ended entries on its next unpaused advance. `update`, `setMapLocation`, `complete`, `fail` and `close` on an entry that is not active log an error and do nothing.
- **Messages.** Both message paths read the entry's `getCommMessageSound()` and build the message at once: the campaign message list (`CampaignUIAPI.addMessage` with the intel) plays the sound and builds a widget that calls `createIntelInfo(MESSAGES)`; `IntelManager.addIntelToTextPanel` plays it and builds the intel list's item widget, which calls `createIntelInfo(INTEL)` (0.98a-RC8 `sources-obf/campaign.comms.java` 3529-3541 and 163-172; 5811-5829 and 6450-6479). `QuestIntel` marks the entry while `show` or an update sends a message, so its rows see mode `update` on both paths. A `sound` argument replaces the update sound for that one message; null keeps it, and an id that is not a top-level key of the merged `sounds.json` is logged and replaced by it. Without an argument the posting message plays `ui_intel_something_posted`, or `ui_intel_major_posted` with `majorPosting()`, and updates play `ui_intel_update`, as `BaseIntelPlugin.getCommMessageSound` does. A message that reports a payment or a loss is an update with the sound `ui_rep_raise` or `ui_rep_drop` and its text in an update row, as vanilla's `PersonBountyIntel` reports its payment as an update bullet ("%s received"); the framework has no plain-message helper, because its text would need a trigger and a text path of its own.
- **Presentation.** The title uses the small insignia font (`setParaSmallInsignia`, then `setParaFontDefault` before the bullets), as vanilla's large mission titles do (`BaseHubMission` with `setUseLargeFontInMissionList`). Bullets keep `BaseIntelPlugin.getBulletColorForMode`: gray in the intel list, text color in the map tooltip, messages and description, as vanilla missions do (`BaseHubMission.addBulletPoints`). `tier` applies while the entry is active; an ending entry sorts as `TIER_COMPLETED`, as `BaseHubMission.getSortTier` does. `important()` follows `BaseHubMission.accept` and `endSuccess`/`endFailure`. `deletable()` adds `addDeleteButton(info, width)` at the end of the description once the entry is completed, failed or closed; `BaseIntelPlugin` shows the confirmation prompt and ends the entry at once. `deletableWhen(check)` adds the same button whenever the named check of the quest passes, also while the entry is active, for an entry the player keeps until deleting it, such as the Cache entry from `CHIP_RECOVERED` on or after failure (check `cacheIntelDeletable` of `KestevenCacheModule`); the check runs with the entry's intel memory and no dialog (`QuestManager.check`), and a check that is not declared throws at load. `faction` falls back to the player's faction, with an error logged once, for an unknown id.
...
- **Saved fields.** The intel manager and the sector's script list save the entry, so `QuestIntel` holds only the quest id, the key, the record, the icon, the tags, the status (`QuestIntel.Status`) and the map entity, and reads its quest's state, declaration and rows when displayed. The update key lives in `BaseIntelPlugin`'s transient `listInfoParam` while a message is built, the message sound in a transient field. When the quest has no state, the entry shows its key as the title, matches no rows and logs once per entry and load.
...
Before matching, `QuestText` writes these keys into a scratch local memory: `$nskr_intel_key` (the entry key), `$nskr_intel_record` (the record, empty for an entry without one), `$nskr_intel_status` (`active`, `completed` or `failed`), `$nskr_intel_update` (the update key, empty otherwise) and `$nskr_intel_mode`. The mode is `list` for the intel list (`ListInfoMode.INTEL`) and the sort title, `tooltip` for the map tooltip (`MAP_TOOLTIP`), `update` for every message: the posting message, updates, completion and failure, in the campaign messages or a dialog's text panel (see Messages above), and a campaign message the UI builds again later (`MESSAGES`), and `desc` for the description panel, its title and its bullets (`IN_DESC`). Rows select with these keys and with `nskr_quest` conditions. Rows under these triggers must use only conditions that work without a dialog: memory keys and `nskr_quest` condition verbs. Their Options are ignored and their Script never runs; `SetTextHighlights` (or `Highlight`) and `SetTextHighlightColors` lines in it are read as highlight declarations.
...
public final class QuestText {
    public static String trigger(String questId, String suffix);                        // nskr_<q> + an intel or raid suffix
    public static Map<String, MemoryAPI> intelMemory(String key, String record, String status, String update, String mode);
    public static Map<String, MemoryAPI> raidMemory(String key);                        // see Raid objectives
...
public final class QuestRewards {
    ...
    public void relationship(String factionId, float delta);  // FactionAPI.adjustRelationship on the player faction
}
```

Each applies the grant the way `AddRemoveCommodity` and `AddRemoveAnyItem` do and prints the vanilla receipt when the context has a dialog. `credits`, `takeCredits` and `storyPoints` need a positive amount (otherwise an error is logged and nothing happens); credits never drop below zero; a negative `commodity` or `item` quantity removes, and `commodity` refreshes `$supplies`-style player memory through `AddRemoveCommodity.updatePlayerMemoryQuantity`. Without a dialog, `storyPoints` uses `addStoryPoints(points)`. `skill` sets the player's skill level; no vanilla command grants a player skill (`NGCSetSkill` is new-game only) and no vanilla helper prints its receipt, so it prints "Gained <skill name>" in small gray text with the name highlighted, then the skill's panel (`TextPanelAPI.beginTooltip().addSkillPanel(person, 10f)` for a person holding only that skill). An unknown skill id or a level of 0 or less logs an error and grants nothing. `relationship` changes the player faction's relationship with a faction by `delta` (`FactionAPI.adjustRelationship`) and, with a dialog, prints the receipt vanilla's `CoreReputationPlugin.adjustPlayerReputation` prints, through `CoreReputationPlugin.addAdjustmentMessage(delta, faction, null, textPanel, null, null, true, 0f)` ("Relationship with ... reduced by 10, currently at ..."); an unknown faction or a change of 0 logs an error. Outside a dialog the quest reports the grant through an intel update instead. Reputation changes stay in rows (`AdjustRep`) when a dialog exists; `relationship` serves hooks without one.
...
- **Marks.** `ctx.mark(entity)` calls `Misc.makeImportant(entity, reason)` with the reason `nskr_<q>`, and the manager calls `Misc.makeUnimportant` when the scope ends, the pattern vanilla hub missions use. Never call `makeImportant` directly for a quest. `ctx.mark(market)` marks the market's own memory with `Misc.makeImportant(market.getMemoryWithoutUpdate(), reason)`; vanilla reads it for the market's primary entity in `Misc.doesMarketHaveMissionImportantPeopleOrIsMarketMissionImportant`.
...
- Presentation (portraits, speaker changes, map markers, highlights, small text, images, sounds) uses vanilla commands; [Presentation in rules](../../../../docs/DIALOGUE.md#presentation-in-rules) names the command for each effect. `confirm` and `engage` do what no vanilla command does.
...
**Vanilla lists.** `VanillaRules.ENGINE` lists every trigger the 0.98a-RC8 game code fires or opens with a literal name (`FireBest.fire`, `FireAll.fire`, the dialog plugins' `fireBest` and `fireAll`, `getBestMatching`, `RuleBasedInteractionDialogPluginImpl`), each with its bundle file and line in the `starsector-knowledge` sources; triggers vanilla fires from variables, such as defeat triggers, are covered because vanilla rows use them. `VanillaRules.COMMAND_PACKAGES` is vanilla's `ruleCommandPackages`. `docs/rules-reference/vanilla-rules-index.txt` lists the triggers vanilla rows use, the literal `FireAll` and `FireBest` targets in vanilla rows, the option ids vanilla rows handle and the memory keys vanilla rows use. The tool reads it from the repository at run time (`VanillaRules.INDEX`); it sits outside `jars/src` because IntelliJ copies every non-Java file of a source root into `Lost.Sector.jar` ([ARCHITECTURE.md](../../../../docs/ARCHITECTURE.md#registration-and-lifecycle)). Regenerate it from a game version's `starsector-core/data/campaign/rules.csv`:

```sh
java -cp "<build output>:<compile jars>" lostsector.quest.dev.RulesCheck --index <vanilla rules.csv> <game version> > docs/rules-reference/vanilla-rules-index.txt
```
...
| Speaker memory key | `$nskr_<q>_<name>` | `$nskr_kq_introduced` |
| Token | `$nskr_<q>_<name>` | `$nskr_kq_job3DaysLeft` |
| Person id | `nskr_<q>_<key>` | `nskr_kq_partyEmployee` |
| Fleet role flag | `$nskr_<q>_<role>` | `$nskr_kq_job3Expedition` |
...
|---|---|---|---|
| [`PayOffEncounter`](#payoffencounter) | Loan collector (quest `ic`), Tri-Tachyon collector (quest `kq`, `KestevenCollector`) | A hostile fleet that demands payment in credits or cargo; pay, part pay or fight | implemented |
| [`BountyEncounter`](#bountyencounter) | Abyss, Eternity, Mothership, Peacekeepers (quest `bounty`) | Placement, first sighting, intel, defeat, reward, completion, guarded entities, shared intel text slots | implemented |
| [`InterceptEncounter`](#interceptencounter) | ARO strike, "LZ" messenger, Auto-Hunter, loan collector (quest `ic`); Tri-Tachyon collector (quest `kq`, `KestevenCollector`) | Daily roll, spawn near the player, the orders of its role, an optional second role | implemented |

A new shared module documents its constructor and behavior here in the commit that adds it. A shared module that keeps data defines a saved record class and an interface the quest's state implements to hold the records.
...
- **Tokens and checks.**
  ...
  | Name | Kind | Value or passes |
  |---|---|---|
  | `<id>FleetName` | token | The fleet's name at spawn |
  | `<id>System`, `<id>Entity` | token | The placement location's system and name |
  | `<id>FleetSystem` | token | The living fleet's current star system; `""` in hyperspace |
  | `<id>InHyperspace` | check | The living fleet is in hyperspace |
  | `<id>Looted` | check | The player looted the beaten fleet while the bounty was active |
  | `<id>Payout`, `<id>PaidAmount` | token | With `payout`: the amount and the credits paid, `Misc.getDGSCredits` |
...
Version 1.0.c breaks saves, so saves from earlier versions need no migration. From 1.0.c on:
...
The old quest code is gone. This table maps each removed owner to its current one, for reading old saves, issues and changelog entries. The Never column of [Rules for every change](#rules-for-every-change) lists the patterns not to bring back.

| Removed | Current owner |
|---|---|
| `QuestStageManager` (stage numbers, `Saved` flags and timers, every-frame polling, per-fleet AI) | `KestevenStage`, `KestevenFlag` and `KestevenState` fields; module hooks; `FleetOrders` on `FleetHelper`. Order comparisons use `KestevenStage.atLeast` |
| `QuestHelper` (questline getters, setters, failure and completion flags, placement helpers) | `KestevenState` through `ctx`, `KestevenQuest` queries, helpers in the owning modules, `SectorLookup` for fixed places |
| `nskr_isKStage`, `nskr_isAtLeastKStage`, `nskr_isAtMostKStage` | `nskr_quest kq is` and `reached` |
| `nskr_kestevenQuest` and the questline's Java dialogs (`GlacierCommsDialog`, `DataSatelliteDialog`, `HintWreckDialog`, `ElizaDialog`, `EndingKestevenDialog`, `EndingElizaDialog`, `CacheDoubtDialog`, `CacheCoreDialog`) with their `CorePlugin` routes; `nskr_job4FleetDialog`, `nskr_elizaInterceptDialog`, `nskr_ttCollectorDialog`, `nskr_altEndingDialogLuddic`, `nskr_altEndingDialogTT` | Rows of the `# KESTEVEN QUESTLINE` blocks with the checks, actions and tokens of the Kesteven modules; claims, `OpenInteractionDialog` rows and fleet role flags ([KESTEVEN_DIALOGUE.md](../../../../docs/quests/KESTEVEN_DIALOGUE.md)) |
| `ElizaRaid`, `ElizaRaidObjectiveCreator` | Raid objective `elizaDisks` of `KestevenElizaModule` |
| `HostileTakeoverBarEvent`, `ElizaSearchBarEvent`, `ElizaSearchSecondBarEvent`, `ElizaSearchFinalBarEvent`, `DelveMeetingBarEvent`, `nskr_barEventFixer` | `AddBarEvents` rows and quest people of `KestevenPartyModule`, `KestevenElizaSearchModule` and `KestevenJob5Module` |
| `EnemyUnknownIntel`, `HostileTakeoverIntel`, `OperationLifesaverIntel`, `TheDelveIntel`, `CacheIntel` | `QuestIntel` keys `job1`, `job3`, `job4`, `job5` and `cache` of the job 1, 3, 4 and 5 modules and `KestevenCacheModule` |
| `KestevenFleets` spawn-and-register code | Builders returning `SimpleFleet`; `ctx.fleets().spawn` |
| Generated people in `KestevenPeople` | `ctx.people()`; `KestevenPeople` keeps the fixed characters' lookups |
| `InterceptManager`, `LoanShark`, `nskr_loanSharkDialog` and the `# DEBT collector dialog` rows | Quest `ic` in `campaign/events/intercepts` (`InterceptEncounter`, `PayOffEncounter`, rows `# INTERCEPTS`) |
| `BlacksiteManager`, `BlacksiteDialog`, `BlacksiteInfo` | Record quest `bs` in `campaign/events/blacksite`; `BlacksiteSpawner` stays world generation |
...
- When a change adds a verified engine fact that rows depend on, put it in the rules guide that owns it and link it from here.
