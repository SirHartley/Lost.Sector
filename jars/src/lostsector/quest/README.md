...
|---|---|---|---|
| Core: definitions, state, store, manager, transitions, marks, claims | `Quest`, `QuestStage`, `QuestState`, `QuestModule`, `Declarations`, `QuestContext`, `QuestManager`, `QuestStore`, `QuestCatalog`, `Quests`, `QuestDialogs`, `NoFlags` | implemented; market marks and `continueDialog` added in T30 | T05, T30 |
...
| Fleets | `QuestFleets`, `QuestFleet`, `FleetRole`, `FleetOrders` | implemented; `keep`, `patrolHomeAfterChase`, `leave(textPrefix)` and `withdrawWhen` added in T22, `defendSystem` in T30 | T07, T22, T30 |
...
| `bounties/abyss` and `bounties/eternity`: spawners, `Saved` counters, loot keys, `AbyssIntel`, `UmbraIntel`, their `BountyLoot` branches | Quest `bounty` in `campaign/bounties`: `BountyEncounter` records, frame sighting, `onLoot` rewards, `QuestIntel` with intel rows (done in T39) |
| `bounties/mothership` and `bounties/peacekeepers`: spawners, intel classes, `MothershipInteractionBlocker` and its `CorePlugin` route, `BountyLoot`, their persistent-data keys and loot keys | Records `mothership` and `peacekeepers` of quest `bounty`: planet claims with `engage`, `FleetOrders.patrolMarkets`, `onLoot` rewards with `QuestRewards.relationship`; `HeliosSite` stays world generation (done in T40) |
| `events/hints/HintManager`, `HintIntel`, `kesteven/tips/KestevenTipBarEventCreator`, `KestevenTipBarEvent`, `KestevenTipIntel`, `enigma/FrostIntel` and the Frost intel code in `HeartOccupation` | Record quest `hint` in `campaign/events/hints`: `onLocationChanged` and `onDay` hooks, the tip as an `AddBarEvents` row with the quest person `officer`, `QuestIntel` entries with records; the bounties report sightings with `HintsQuest.reportBountySighted` (done in T45; see [HINTS.md](../../../../docs/quests/HINTS.md)) |
