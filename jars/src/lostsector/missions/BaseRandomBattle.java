...
import lostsector.missions.FleetGenerator.GeneratorFleetTypes;

import java.awt.*;
import java.util.List;
import java.util.*;

public class BaseRandomBattle implements MissionDefinitionPlugin {
    ...
    protected static FleetDataAPI generateFleet(int maxPts, float qualityFactor, float opBonus, int avgSMods, FleetSide side,
            String faction, String fleetType, MissionDefinitionAPI api, long seed, boolean autofit) {
        ...
        return type.generate(api, side, faction, qualityFactor, opBonus, avgSMods, maxPts, seed, autofit);
    }
    ...
    private void init() {
		...
		FACTIONS.add("enigma", 0.25f);
		FACTIONS.add("kesteven", 0.25f);
        FACTIONS.add("prot_ops", 0.25f);
    }
